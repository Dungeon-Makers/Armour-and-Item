...
public class FoxEntity extends AnimalEntity {
   ...
   protected void registerGoals() {
      ...
      this.targetSelector.addGoal(3, new FoxEntity.RevengeGoal(LivingEntity.class, false, false, (p_213493_1_) -> {
         return TRUSTED_TARGET_SELECTOR.test(p_213493_1_) && !this.trusts(p_213493_1_.getUUID());
      }));
   }
   ...
   public void handleEntityEvent(byte p_70103_1_) {
      if (p_70103_1_ == 45) {
         ...
         if (!itemstack.isEmpty()) {
            for(int i = 0; i < 8; ++i) {
               Vec3d vec3d = (new Vec3d(((double)this.random.nextFloat() - 0.5D) * 0.1D, Math.random() * 0.1D + 0.1D, 0.0D)).xRot(-this.xRot * ((float)Math.PI / 180F)).yRot(-this.yRot * ((float)Math.PI / 180F));
   ...
   public FoxEntity func_90011_a(AgeableEntity p_90011_1_) {
      FoxEntity foxentity = EntityType.FOX.create(this.level);
      foxentity.setFoxType(this.random.nextBoolean() ? this.getFoxType() : ((FoxEntity)p_90011_1_).getFoxType());
      return foxentity;
   }
   ...
   public void addAdditionalSaveData(CompoundNBT p_213281_1_) {
      ...
      for(UUID uuid : list) {
         if (uuid != null) {
            listnbt.add(NBTUtil.func_186862_a(uuid));
         }
      }
   ...
   public void readAdditionalSaveData(CompoundNBT p_70037_1_) {
      ...
      for(int i = 0; i < listnbt.size(); ++i) {
         this.addTrustedUUID(NBTUtil.loadUUID(listnbt.getCompound(i)));
      }
   ...
   class MateGoal extends BreedGoal {
      ...
      protected void breed() {
         FoxEntity foxentity = (FoxEntity)this.animal.func_90011_a(this.partner);
         if (foxentity != null) {
   ...
   public class PounceGoal extends net.minecraft.entity.ai.goal.JumpGoal {
      ...
      public void start() {
         ...
         FoxEntity.this.setDeltaMovement(FoxEntity.this.getDeltaMovement().add(vec3d.x * 0.8D, 0.9D, vec3d.z * 0.8D));
         FoxEntity.this.getNavigation().stop();
      }
      ...
      public void tick() {
         ...
         if (!FoxEntity.this.isFaceplanted()) {
            Vec3d vec3d = FoxEntity.this.getDeltaMovement();
            if (vec3d.y * vec3d.y < (double)0.03F && FoxEntity.this.xRot != 0.0F) {
         ...
         } else if (FoxEntity.this.xRot > 0.0F && FoxEntity.this.onGround && (float)FoxEntity.this.getDeltaMovement().y != 0.0F && FoxEntity.this.level.getBlockState(new BlockPos(FoxEntity.this)).getBlock() == Blocks.SNOW) {
   ...
   class RevengeGoal extends NearestAttackableTargetGoal<LivingEntity> {
      ...
      public void start() {
         FoxEntity.this.setTarget(this.trustedLastHurtBy);
   ...
   class SwimGoal extends net.minecraft.entity.ai.goal.SwimGoal {
      ...
      public boolean canUse() {
         return FoxEntity.this.isInWater() && FoxEntity.this.func_212107_bY() > 0.25D || FoxEntity.this.isInLava();
      }
   }
   ...
   public static enum Type {
      ...
      private Type(int p_i50721_3_, String p_i50721_4_, Biome... p_i50721_5_) {
         ...
         this.biomes = Arrays.asList(p_i50721_5_);
      }
