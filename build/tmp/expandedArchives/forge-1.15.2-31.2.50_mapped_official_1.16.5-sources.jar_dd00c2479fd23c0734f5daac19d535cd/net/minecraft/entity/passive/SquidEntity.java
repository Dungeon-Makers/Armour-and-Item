...
public class SquidEntity extends WaterMobEntity {
   ...
   public void aiStep() {
      ...
      if (this.isInWaterOrBubble()) {
         ...
         this.xBodyRot += (-((float)MathHelper.atan2((double)f1, vec3d.y)) * (180F / (float)Math.PI) - this.xBodyRot) * 0.1F;
      } else {
   ...
   private Vec3d rotateVector(Vec3d p_207400_1_) {
      Vec3d vec3d = p_207400_1_.xRot(this.xBodyRotO * ((float)Math.PI / 180F));
   ...
   private void spawnInk() {
      ...
      for(int i = 0; i < 30; ++i) {
         ...
         ((ServerWorld)this.level).sendParticles(ParticleTypes.SQUID_INK, vec3d.x, vec3d.y + 0.5D, vec3d.z, 0, vec3d2.x, vec3d2.y, vec3d2.z, (double)0.1F);
      }
   ...
   class FleeGoal extends Goal {
      ...
      public void tick() {
         ...
         if (livingentity != null) {
            ...
            BlockState blockstate = SquidEntity.this.level.getBlockState(new BlockPos(SquidEntity.this.getX() + vec3d.x, SquidEntity.this.getY() + vec3d.y, SquidEntity.this.getZ() + vec3d.z));
            IFluidState ifluidstate = SquidEntity.this.level.getFluidState(new BlockPos(SquidEntity.this.getX() + vec3d.x, SquidEntity.this.getY() + vec3d.y, SquidEntity.this.getZ() + vec3d.z));
            if (ifluidstate.is(FluidTags.WATER) || blockstate.isAir()) {
