...
public final class ProjectileHelper {
   public static RayTraceResult func_221266_a(Entity p_221266_0_, boolean p_221266_1_, boolean p_221266_2_, @Nullable Entity p_221266_3_, RayTraceContext.BlockMode p_221266_4_) {
      return func_221268_a(p_221266_0_, p_221266_1_, p_221266_2_, p_221266_3_, p_221266_4_, true, (p_221270_2_) -> {
         return !p_221270_2_.isSpectator() && p_221270_2_.isPickable() && (p_221266_2_ || !p_221270_2_.is(p_221266_3_)) && !p_221270_2_.noPhysics;
      }, p_221266_0_.getBoundingBox().expandTowards(p_221266_0_.getDeltaMovement()).inflate(1.0D));
   }
   ...
   public static EntityRayTraceResult func_221271_a(World p_221271_0_, Entity p_221271_1_, Vec3d p_221271_2_, Vec3d p_221271_3_, AxisAlignedBB p_221271_4_, Predicate<Entity> p_221271_5_) {
      return getEntityHitResult(p_221271_0_, p_221271_1_, p_221271_2_, p_221271_3_, p_221271_4_, p_221271_5_, Double.MAX_VALUE);
   }
   ...
   private static RayTraceResult func_221268_a(Entity p_221268_0_, boolean p_221268_1_, boolean p_221268_2_, @Nullable Entity p_221268_3_, RayTraceContext.BlockMode p_221268_4_, boolean p_221268_5_, Predicate<Entity> p_221268_6_, AxisAlignedBB p_221268_7_) {
      Vec3d vec3d = p_221268_0_.getDeltaMovement();
      World world = p_221268_0_.level;
      Vec3d vec3d1 = p_221268_0_.position();
      if (p_221268_5_ && !world.func_226662_a_(p_221268_0_, p_221268_0_.getBoundingBox(), (Set<Entity>)(!p_221268_2_ && p_221268_3_ != null ? func_211325_a(p_221268_3_) : ImmutableSet.of()))) {
         return new BlockRayTraceResult(vec3d1, Direction.getNearest(vec3d.x, vec3d.y, vec3d.z), new BlockPos(p_221268_0_), false);
      } else {
         Vec3d vec3d2 = vec3d1.add(vec3d);
         RayTraceResult raytraceresult = world.clip(new RayTraceContext(vec3d1, vec3d2, p_221268_4_, RayTraceContext.FluidMode.NONE, p_221268_0_));
         if (p_221268_1_) {
   ...
   @Nullable
   @OnlyIn(Dist.CLIENT)
   public static EntityRayTraceResult getEntityHitResult(Entity p_221273_0_, Vec3d p_221273_1_, Vec3d p_221273_2_, AxisAlignedBB p_221273_3_, Predicate<Entity> p_221273_4_, double p_221273_5_) {
      ...
      for(Entity entity1 : world.getEntities(p_221273_0_, p_221273_3_, p_221273_4_)) {
         ...
         Optional<Vec3d> optional = axisalignedbb.clip(p_221273_1_, p_221273_2_);
         if (axisalignedbb.contains(p_221273_1_)) {
            ...
            double d1 = p_221273_1_.distanceToSqr(vec3d1);
            if (d1 < d0 || d0 == 0.0D) {
   ...
   @Nullable
   public static EntityRayTraceResult getEntityHitResult(World p_221269_0_, Entity p_221269_1_, Vec3d p_221269_2_, Vec3d p_221269_3_, AxisAlignedBB p_221269_4_, Predicate<Entity> p_221269_5_, double p_221269_6_) {
      ...
      for(Entity entity1 : p_221269_0_.getEntities(p_221269_1_, p_221269_4_, p_221269_5_)) {
         ...
         Optional<Vec3d> optional = axisalignedbb.clip(p_221269_2_, p_221269_3_);
   ...
   private static Set<Entity> func_211325_a(Entity p_211325_0_) {
      Entity entity = p_211325_0_.getVehicle();
   ...
   public static final void rotateTowardsMovement(Entity p_188803_0_, float p_188803_1_) {
      Vec3d vec3d = p_188803_0_.getDeltaMovement();
      float f = MathHelper.sqrt(Entity.getHorizontalDistanceSqr(vec3d));
      p_188803_0_.yRot = (float)(MathHelper.atan2(vec3d.z, vec3d.x) * (double)(180F / (float)Math.PI)) + 90.0F;

      for(p_188803_0_.xRot = (float)(MathHelper.atan2((double)f, vec3d.y) * (double)(180F / (float)Math.PI)) - 90.0F; p_188803_0_.xRot - p_188803_0_.xRotO < -180.0F; p_188803_0_.xRotO -= 360.0F) {
         ;
      }
