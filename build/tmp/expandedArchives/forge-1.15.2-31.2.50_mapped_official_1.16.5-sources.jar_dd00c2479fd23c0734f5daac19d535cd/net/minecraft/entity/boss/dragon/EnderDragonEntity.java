...
public class EnderDragonEntity extends MobEntity implements IMob {
   ...
   public EnderDragonEntity(EntityType<? extends EnderDragonEntity> p_i50230_1_, World p_i50230_2_) {
      ...
      this.noPhysics = true;
      this.noCulling = true;
      if (!p_i50230_2_.isClientSide && p_i50230_2_.dimension instanceof EndDimension) {
         this.dragonFight = ((EndDimension)p_i50230_2_.dimension).func_186063_s();
      } else {
   ...
   protected void func_110147_ax() {
      ...
      this.getAttribute(SharedMonsterAttributes.field_111267_a).setBaseValue(200.0D);
   }
   ...
   public void aiStep() {
      ...
      if (this.getHealth() <= 0.0F) {
         ...
         float f12 = 0.2F / (MathHelper.sqrt(getHorizontalDistanceSqr(vec3d4)) * 10.0F + 1.0F);
         ...
         if (this.isNoAi()) {
            ...
            if (this.level.isClientSide) {
               ...
               Vec3d vec3d = iphase.getFlyTargetLocation();
               if (vec3d != null) {
                  double d8 = vec3d.x - this.getX();
                  double d9 = vec3d.y - this.getY();
                  double d10 = vec3d.z - this.getZ();
                  ...
                  Vec3d vec3d1 = vec3d.subtract(this.getX(), this.getY(), this.getZ()).normalize();
                  Vec3d vec3d2 = (new Vec3d((double)MathHelper.sin(this.yRot * ((float)Math.PI / 180F)), this.getDeltaMovement().y, (double)(-MathHelper.cos(this.yRot * ((float)Math.PI / 180F))))).normalize();
                  ...
                  Vec3d vec3d3 = this.getDeltaMovement().normalize();
            ...
            for(int l = 0; l < this.subEntities.length; ++l) {
               this.subEntities[l].xo = avec3d[l].x;
               this.subEntities[l].yo = avec3d[l].y;
               this.subEntities[l].zo = avec3d[l].z;
               this.subEntities[l].xOld = avec3d[l].x;
               this.subEntities[l].yOld = avec3d[l].y;
               this.subEntities[l].zOld = avec3d[l].z;
            }
         ...
         for(int l1 = j; l1 <= i1; ++l1) {
            for(int i2 = k; i2 <= j1; ++i2) {
               ...
               if (!blockstate.isAir(this.level, blockpos) && blockstate.getMaterial() != Material.FIRE) {
                  if (net.minecraftforge.common.ForgeHooks.canEntityDestroy(this.level, blockpos, this) && !BlockTags.DRAGON_IMMUNE.func_199685_a_(block)) {
   ...
   public Vec3d getHeadLookVector(float p_184665_1_) {
      IPhase iphase = this.phaseManager.getCurrentPhase();
      PhaseType<? extends IPhase> phasetype = iphase.getPhase();
      Vec3d vec3d;
      if (phasetype != PhaseType.LANDING && phasetype != PhaseType.TAKEOFF) {
         if (iphase.isSitting()) {
            ...
            float f5 = 1.5F;
            this.xRot = -45.0F;
            vec3d = this.getViewVector(p_184665_1_);
            this.xRot = f4;
         } else {
            vec3d = this.getViewVector(p_184665_1_);
         }
