...
public class StrafePlayerPhase extends Phase {
      ...
      if (this.attackTarget == null) {
         ...
         if (this.attackTarget.distanceToSqr(this.dragon) < 4096.0D) {
            if (this.dragon.canSee(this.attackTarget)) {
               ++this.fireballCharge;
               Vec3d vec3d1 = (new Vec3d(this.attackTarget.getX() - this.dragon.getX(), 0.0D, this.attackTarget.getZ() - this.dragon.getZ())).normalize();
               Vec3d vec3d = (new Vec3d((double)MathHelper.sin(this.dragon.yRot * ((float)Math.PI / 180F)), 0.0D, (double)(-MathHelper.cos(this.dragon.yRot * ((float)Math.PI / 180F))))).normalize();
               float f1 = (float)vec3d.dot(vec3d1);
               ...
               if (this.fireballCharge >= 5 && f >= 0.0F && f < 10.0F) {
                  ...
                  this.dragon.level.levelEvent((PlayerEntity)null, 1017, new BlockPos(this.dragon), 0);
   ...
   private void navigateToNextPathNode() {
      if (this.currentPath != null && !this.currentPath.isDone()) {
         ...
         double d1;
         while(true) {
            d1 = vec3d.y + (double)(this.dragon.getRandom().nextFloat() * 20.0F);
            if (!(d1 < vec3d.y)) {
               break;
            }
         }
