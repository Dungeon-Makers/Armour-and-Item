...
public class WalkToHouseTask extends Task<LivingEntity> {
   ...
   protected boolean checkExtraStartConditions(ServerWorld p_212832_1_, LivingEntity p_212832_2_) {
      if (p_212832_1_.getGameTime() - this.lastUpdate < 20L) {
         ...
         Optional<BlockPos> optional = pointofinterestmanager.func_219147_b(PointOfInterestType.HOME.getPredicate(), (p_220522_0_) -> {
            return true;
         }, new BlockPos(p_212832_2_), 48, PointOfInterestManager.Status.ANY);
         return optional.isPresent() && !(optional.get().distSqr(new BlockPos(creatureentity)) <= 4.0D);
      }
   }
   ...
   protected void start(ServerWorld p_212831_1_, LivingEntity p_212831_2_, long p_212831_3_) {
      ...
      Stream<BlockPos> stream = pointofinterestmanager.findAll(PointOfInterestType.HOME.getPredicate(), predicate, new BlockPos(p_212831_2_), 48, PointOfInterestManager.Status.ANY);
