...
public class MoveTowardsRaidGoal<T extends AbstractRaiderEntity> extends Goal {
   ...
   public boolean canUse() {
      return this.mob.getTarget() == null && !this.mob.isVehicle() && this.mob.hasActiveRaid() && !this.mob.getCurrentRaid().isOver() && !((ServerWorld)this.mob.level).isVillage(new BlockPos(this.mob));
   }

   public boolean canContinueToUse() {
      return this.mob.hasActiveRaid() && !this.mob.getCurrentRaid().isOver() && this.mob.level instanceof ServerWorld && !((ServerWorld)this.mob.level).isVillage(new BlockPos(this.mob));
   }
   ...
   public void tick() {
      if (this.mob.hasActiveRaid()) {
         ...
         if (!this.mob.isPathFinding()) {
            Vec3d vec3d = RandomPositionGenerator.getPosTowards(this.mob, 15, 4, new Vec3d(raid.getCenter()));
            if (vec3d != null) {
