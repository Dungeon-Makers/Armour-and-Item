...
public class WalkToPOITask extends Task<VillagerEntity> {
   ...
   protected boolean checkExtraStartConditions(ServerWorld p_212832_1_, VillagerEntity p_212832_2_) {
      return !p_212832_1_.isVillage(new BlockPos(p_212832_2_));
   }
   ...
   protected void start(ServerWorld p_212831_1_, VillagerEntity p_212831_2_, long p_212831_3_) {
      ...
      int i = pointofinterestmanager.sectionsToVillage(SectionPos.of(new BlockPos(p_212831_2_)));
      Vec3d vec3d = null;

      for(int j = 0; j < 5; ++j) {
         Vec3d vec3d1 = RandomPositionGenerator.getLandPos(p_212831_2_, 15, 7, (p_225444_1_) -> {
            return (double)(-p_212831_1_.sectionsToVillage(SectionPos.of(p_225444_1_)));
         });
         if (vec3d1 != null) {
            int k = pointofinterestmanager.sectionsToVillage(SectionPos.of(new BlockPos(vec3d1)));
            if (k < i) {
               vec3d = vec3d1;
               break;
            }
      ...
      if (vec3d != null) {
         p_212831_2_.getBrain().setMemory(MemoryModuleType.WALK_TARGET, new WalkTarget(vec3d, this.speedModifier, this.closeEnoughDistance));
      }
