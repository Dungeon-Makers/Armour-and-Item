...
public class FollowBoatGoal extends Goal {
   ...
   public boolean canUse() {
      ...
      for(BoatEntity boatentity : list) {
         ...
         if (entity instanceof LivingEntity && (MathHelper.abs(((LivingEntity)entity).xxa) > 0.0F || MathHelper.abs(((LivingEntity)entity).zza) > 0.0F)) {
            flag = true;
            break;
         }
      }
   ...
   public void start() {
      for(BoatEntity boatentity : this.mob.level.getEntitiesOfClass(BoatEntity.class, this.mob.getBoundingBox().inflate(5.0D))) {
         if (boatentity.getControllingPassenger() != null && boatentity.getControllingPassenger() instanceof LivingEntity) {
            this.following = (LivingEntity)boatentity.getControllingPassenger();
            break;
         }
      }
   ...
   public void tick() {
      ...
      float f = this.currentGoal == BoatGoals.GO_IN_BOAT_DIRECTION ? (flag ? 0.17999999F : 0.0F) : 0.135F;
      ...
      if (--this.timeToRecalcPath <= 0) {
         ...
         if (this.currentGoal == BoatGoals.GO_TO_BOAT) {
            ...
            BlockPos blockpos1 = (new BlockPos(this.following)).relative(direction, 10);
