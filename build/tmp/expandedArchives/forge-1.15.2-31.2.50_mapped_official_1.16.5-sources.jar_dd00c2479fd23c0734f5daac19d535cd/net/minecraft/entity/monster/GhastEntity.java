...
public class GhastEntity extends FlyingEntity implements IMob {
   ...
   protected void func_110147_ax() {
      ...
      this.getAttribute(SharedMonsterAttributes.field_111265_b).setBaseValue(100.0D);
   }
      ...
      public void tick() {
         ...
         if (livingentity.distanceToSqr(this.ghast) < 4096.0D && this.ghast.canSee(livingentity)) {
            ...
            if (this.chargeTime == 10) {
               world.levelEvent((PlayerEntity)null, 1015, new BlockPos(this.ghast), 0);
            }

            if (this.chargeTime == 20) {
               double d1 = 4.0D;
               Vec3d vec3d = this.ghast.getViewVector(1.0F);
               double d2 = livingentity.getX() - (this.ghast.getX() + vec3d.x * 4.0D);
               double d3 = livingentity.getY(0.5D) - (0.5D + this.ghast.getY(0.5D));
               double d4 = livingentity.getZ() - (this.ghast.getZ() + vec3d.z * 4.0D);
               world.levelEvent((PlayerEntity)null, 1016, new BlockPos(this.ghast), 0);
               ...
               fireballentity.setPos(this.ghast.getX() + vec3d.x * 4.0D, this.ghast.getY(0.5D) + 0.5D, fireballentity.getZ() + vec3d.z * 4.0D);
   ...
   static class LookAroundGoal extends Goal {
      ...
      public void tick() {
         if (this.ghast.getTarget() == null) {
            ...
            this.ghast.yRot = -((float)MathHelper.atan2(vec3d.x, vec3d.z)) * (180F / (float)Math.PI);
