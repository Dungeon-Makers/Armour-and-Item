...
public class GuardianEntity extends MonsterEntity {
   ...
   protected void func_110147_ax() {
      ...
      this.getAttribute(SharedMonsterAttributes.field_111264_e).setBaseValue(6.0D);
      this.getAttribute(SharedMonsterAttributes.field_111263_d).setBaseValue(0.5D);
      this.getAttribute(SharedMonsterAttributes.field_111265_b).setBaseValue(16.0D);
      ...
      if (this.isAlive()) {
         if (this.level.isClientSide) {
            ...
            if (!this.isInWater()) {
               ...
               this.clientSideTouchedGround = vec3d.y < 0.0D && this.level.loadedAndEntityCanStandOn((new BlockPos(this)).below(), this);
            ...
            if (this.isMoving() && this.isInWater()) {
               ...
               for(int i = 0; i < 2; ++i) {
                  this.level.addParticle(ParticleTypes.BUBBLE, this.getRandomX(0.5D) - vec3d1.x * 1.5D, this.getRandomY() - vec3d1.y * 1.5D, this.getRandomZ(0.5D) - vec3d1.z * 1.5D, 0.0D, 0.0D, 0.0D);
               }
            }
      ...
      public void tick() {
         ...
         if (!this.guardian.canSee(livingentity)) {
            ...
            if (this.attackTime == 0) {
               ...
               livingentity.hurt(DamageSource.mobAttack(this.guardian), (float)this.guardian.getAttribute(SharedMonsterAttributes.field_111264_e).getValue());
               this.guardian.setTarget((LivingEntity)null);
            }
   ...
   static class MoveHelperController extends MovementController {
      ...
      public void tick() {
         if (this.operation == MovementController.Action.MOVE_TO && !this.guardian.getNavigation().isDone()) {
            ...
            float f1 = (float)(this.speedModifier * this.guardian.getAttribute(SharedMonsterAttributes.field_111263_d).getValue());
