...
public class ElderGuardianEntity extends GuardianEntity {
   ...
   protected void func_110147_ax() {
      ...
      this.getAttribute(SharedMonsterAttributes.field_111264_e).setBaseValue(8.0D);
      this.getAttribute(SharedMonsterAttributes.field_111267_a).setBaseValue(80.0D);
   }
      ...
      if ((this.tickCount + this.getId()) % 1200 == 0) {
         ...
         for(ServerPlayerEntity serverplayerentity : list) {
            if (!serverplayerentity.hasEffect(effect) || serverplayerentity.getEffect(effect).getAmplifier() < 2 || serverplayerentity.getEffect(effect).getDuration() < 1200) {
               serverplayerentity.connection.send(new SChangeGameStatePacket(10, 0.0F));
