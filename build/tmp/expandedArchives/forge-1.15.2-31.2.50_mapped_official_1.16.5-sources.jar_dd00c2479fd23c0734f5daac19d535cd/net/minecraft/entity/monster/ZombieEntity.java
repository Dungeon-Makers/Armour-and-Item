...
public class ZombieEntity extends MonsterEntity {
   ...
   protected void addBehaviourGoals() {
      ...
      this.targetSelector.addGoal(1, (new HurtByTargetGoal(this)).setAlertOthers(ZombiePigmanEntity.class));
   ...
   protected void func_110147_ax() {
      ...
      this.getAttribute(SharedMonsterAttributes.field_111265_b).setBaseValue(35.0D);
      this.getAttribute(SharedMonsterAttributes.field_111263_d).setBaseValue((double)0.23F);
      this.getAttribute(SharedMonsterAttributes.field_111264_e).setBaseValue(3.0D);
      this.getAttribute(SharedMonsterAttributes.field_188791_g).setBaseValue(2.0D);
      this.func_110140_aT().func_111150_b(field_110186_bp).setBaseValue(this.random.nextDouble() * net.minecraftforge.common.ForgeConfig.SERVER.zombieBaseSummonChance.get());
   }
   ...
   public void setBaby(boolean p_82227_1_) {
      ...
      if (this.level != null && !this.level.isClientSide) {
         ...
         iattributeinstance.removeModifier(SPEED_MODIFIER_BABY);
         if (p_82227_1_) {
            iattributeinstance.addModifier(SPEED_MODIFIER_BABY);
         }
      }
   ...
   protected void doUnderWaterConversion() {
      ...
      this.level.levelEvent((PlayerEntity)null, 1040, new BlockPos(this), 0);
   }

   protected void func_213698_b(EntityType<? extends ZombieEntity> p_213698_1_) {
      if (!this.removed) {
         ZombieEntity zombieentity = p_213698_1_.create(this.level);
         ...
         zombieentity.handleAttributes(zombieentity.level.getCurrentDifficultyAt(new BlockPos(zombieentity)).getSpecialMultiplier());
         zombieentity.setBaby(this.isBaby());
         zombieentity.setNoAi(this.isNoAi());

         for(EquipmentSlotType equipmentslottype : EquipmentSlotType.values()) {
            ItemStack itemstack = this.getItemBySlot(equipmentslottype);
            if (!itemstack.isEmpty()) {
               zombieentity.setItemSlot(equipmentslottype, itemstack.copy());
               zombieentity.setDropChance(equipmentslottype, this.getEquipmentDropChance(equipmentslottype));
         ...
         this.level.addFreshEntity(zombieentity);
         this.remove();
      }
   }
   ...
   public boolean func_184645_a(PlayerEntity p_184645_1_, Hand p_184645_2_) {
      ...
      if (item instanceof SpawnEggItem && ((SpawnEggItem)item).spawnsEntity(itemstack.getTag(), this.getType())) {
         if (!this.level.isClientSide) {
            ZombieEntity zombieentity = (ZombieEntity)this.getType().create(this.level);
            if (zombieentity != null) {
               zombieentity.setBaby(true);
               zombieentity.moveTo(this.getX(), this.getY(), this.getZ(), 0.0F, 0.0F);
               this.level.addFreshEntity(zombieentity);
               if (itemstack.hasCustomHoverName()) {
                  zombieentity.setCustomName(itemstack.getHoverName());
               }
   ...
   public boolean hurt(DamageSource p_70097_1_, float p_70097_2_) {
      if (super.hurt(p_70097_1_, p_70097_2_)) {
         ...
         net.minecraftforge.event.entity.living.ZombieEvent.SummonAidEvent event = net.minecraftforge.event.ForgeEventFactory.fireZombieSummonAid(this, level, i, j, k, livingentity, this.getAttribute(field_110186_bp).getValue());
         if (event.getResult() == net.minecraftforge.eventbus.api.Event.Result.DENY) return true;
         if (event.getResult() == net.minecraftforge.eventbus.api.Event.Result.ALLOW  ||
            livingentity != null && this.level.getDifficulty() == Difficulty.HARD && (double)this.random.nextFloat() < this.getAttribute(field_110186_bp).getValue() && this.level.getGameRules().getBoolean(GameRules.RULE_DOMOBSPAWNING)) {
            ...
            for(int l = 0; l < 50; ++l) {
               ...
               if (this.level.getBlockState(blockpos).entityCanStandOnFace(this.level, blockpos, zombieentity) && this.level.getMaxLocalRawBrightness(new BlockPos(i1, j1, k1)) < 10) {
                  ...
                  if (!this.level.hasNearbyAlivePlayer((double)i1, (double)j1, (double)k1, 7.0D) && this.level.isUnobstructed(zombieentity) && this.level.noCollision(zombieentity) && !this.level.containsAnyLiquid(zombieentity.getBoundingBox())) {
                     ...
                     zombieentity.finalizeSpawn(this.level, this.level.getCurrentDifficultyAt(new BlockPos(zombieentity)), SpawnReason.REINFORCEMENT, (ILivingEntityData)null, (CompoundNBT)null);
                     this.getAttribute(field_110186_bp).addModifier(new AttributeModifier("Zombie reinforcement caller charge", (double)-0.05F, AttributeModifier.Operation.ADDITION));
                     zombieentity.getAttribute(field_110186_bp).addModifier(new AttributeModifier("Zombie reinforcement callee charge", (double)-0.05F, AttributeModifier.Operation.ADDITION));
                     break;
                  }
               }
            }
   ...
   public void func_70074_a(LivingEntity p_70074_1_) {
      ...
      if ((this.level.getDifficulty() == Difficulty.NORMAL || this.level.getDifficulty() == Difficulty.HARD) && p_70074_1_ instanceof VillagerEntity) {
         ...
         zombievillagerentity.finalizeSpawn(this.level, this.level.getCurrentDifficultyAt(new BlockPos(zombievillagerentity)), SpawnReason.CONVERSION, new ZombieEntity.GroupData(false), (CompoundNBT)null);
         zombievillagerentity.setVillagerData(villagerentity.getVillagerData());
         zombievillagerentity.setGossips(villagerentity.getGossips().func_220914_a(NBTDynamicOps.INSTANCE).getValue());
         ...
         if (this.isPersistenceRequired()) {
            zombievillagerentity.setPersistenceRequired();
         }

         zombievillagerentity.setInvulnerable(this.isInvulnerable());
         this.level.addFreshEntity(zombievillagerentity);
         this.level.levelEvent((PlayerEntity)null, 1026, new BlockPos(this), 0);
      }
   ...
   protected boolean canHoldItem(ItemStack p_175448_1_) {
   ...
   protected void handleAttributes(float p_207304_1_) {
      this.getAttribute(SharedMonsterAttributes.field_111266_c).addModifier(new AttributeModifier("Random spawn bonus", this.random.nextDouble() * (double)0.05F, AttributeModifier.Operation.ADDITION));
      double d0 = this.random.nextDouble() * 1.5D * (double)p_207304_1_;
      if (d0 > 1.0D) {
         this.getAttribute(SharedMonsterAttributes.field_111265_b).addModifier(new AttributeModifier("Random zombie-spawn bonus", d0, AttributeModifier.Operation.MULTIPLY_TOTAL));
      }

      if (this.random.nextFloat() < p_207304_1_ * 0.05F) {
         this.getAttribute(field_110186_bp).addModifier(new AttributeModifier("Leader zombie bonus", this.random.nextDouble() * 0.25D + 0.5D, AttributeModifier.Operation.ADDITION));
         this.getAttribute(SharedMonsterAttributes.field_111267_a).addModifier(new AttributeModifier("Leader zombie bonus", this.random.nextDouble() * 3.0D + 1.0D, AttributeModifier.Operation.MULTIPLY_TOTAL));
