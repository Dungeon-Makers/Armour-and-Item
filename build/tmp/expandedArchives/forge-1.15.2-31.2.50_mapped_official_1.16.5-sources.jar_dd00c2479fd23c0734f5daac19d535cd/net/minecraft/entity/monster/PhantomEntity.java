...
public class PhantomEntity extends FlyingEntity implements IMob {
   ...
   private void updatePhantomSizeInfo() {
      ...
      this.getAttribute(SharedMonsterAttributes.field_111264_e).setBaseValue((double)(6 + this.getPhantomSize()));
   }
         ...
         if (this.nextScanTick > 0) {
            ...
            if (!list.isEmpty()) {
               list.sort((p_203140_0_, p_203140_1_) -> {
                  return p_203140_0_.getY() > p_203140_1_.getY() ? -1 : 1;
               });
   ...
   class MoveHelperController extends MovementController {
      ...
      public void tick() {
         ...
         Vec3d vec3d = PhantomEntity.this.getDeltaMovement();
         PhantomEntity.this.setDeltaMovement(vec3d.add((new Vec3d(d3, d5, d4)).subtract(vec3d).scale(0.2D)));
      }
   }
   ...
   class OrbitPointGoal extends PhantomEntity.MoveGoal {
      ...
      public void tick() {
         ...
         if (PhantomEntity.this.moveTargetPoint.y < PhantomEntity.this.getY() && !PhantomEntity.this.level.isEmptyBlock((new BlockPos(PhantomEntity.this)).below(1))) {
            this.height = Math.max(1.0F, this.height);
            this.selectNext();
         }

         if (PhantomEntity.this.moveTargetPoint.y > PhantomEntity.this.getY() && !PhantomEntity.this.level.isEmptyBlock((new BlockPos(PhantomEntity.this)).above(1))) {
      ...
      private void selectNext() {
         if (BlockPos.ZERO.equals(PhantomEntity.this.anchorPoint)) {
            PhantomEntity.this.anchorPoint = new BlockPos(PhantomEntity.this);
         }

         this.angle += this.clockwise * 15.0F * ((float)Math.PI / 180F);
         PhantomEntity.this.moveTargetPoint = (new Vec3d(PhantomEntity.this.anchorPoint)).add((double)(this.distance * MathHelper.cos(this.angle)), (double)(-4.0F + this.height), (double)(this.distance * MathHelper.sin(this.angle)));
      }
   }
   ...
   class PickAttackGoal extends Goal {
      ...
      private void setAnchorAboveTarget() {
         PhantomEntity.this.anchorPoint = (new BlockPos(PhantomEntity.this.getTarget())).above(20 + PhantomEntity.this.random.nextInt(20));
   ...
   class SweepAttackGoal extends PhantomEntity.MoveGoal {
      ...
      public void tick() {
         LivingEntity livingentity = PhantomEntity.this.getTarget();
         PhantomEntity.this.moveTargetPoint = new Vec3d(livingentity.getX(), livingentity.getY(0.5D), livingentity.getZ());
         if (PhantomEntity.this.getBoundingBox().inflate((double)0.2F).intersects(livingentity.getBoundingBox())) {
            ...
            PhantomEntity.this.attackPhase = PhantomEntity.AttackPhase.CIRCLE;
            PhantomEntity.this.level.levelEvent(1039, new BlockPos(PhantomEntity.this), 0);
