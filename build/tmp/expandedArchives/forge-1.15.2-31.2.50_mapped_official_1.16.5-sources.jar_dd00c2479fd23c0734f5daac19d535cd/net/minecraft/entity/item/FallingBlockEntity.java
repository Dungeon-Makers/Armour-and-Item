   ...
   public void tick() {
      if (this.blockState.isAir()) {
         ...
         if (!this.level.isClientSide) {
            ...
            if (flag && d0 > 1.0D) {
               BlockRayTraceResult blockraytraceresult = this.level.clip(new RayTraceContext(new Vec3d(this.xo, this.yo, this.zo), this.position(), RayTraceContext.BlockMode.COLLIDER, RayTraceContext.FluidMode.SOURCE_ONLY, this));
            ...
            if (!this.onGround && !flag1) {
               ...
               if (blockstate.getBlock() != Blocks.MOVING_PISTON) {
                  ...
                  if (!this.cancelDrop) {
                     ...
                     if (flag2 && flag4) {
                        if (this.blockState.func_196959_b(BlockStateProperties.WATERLOGGED) && this.level.getFluidState(blockpos1).getType() == Fluids.WATER) {
                           this.blockState = this.blockState.setValue(BlockStateProperties.WATERLOGGED, Boolean.valueOf(true));
                        }

                        if (this.level.setBlock(blockpos1, this.blockState, 3)) {
                           if (block instanceof FallingBlock) {
                              ((FallingBlock)block).onLand(this.level, blockpos1, this.blockState, blockstate);
                           }
                     ...
                     ((FallingBlock)block).onBroken(this.level, blockpos1);
                  }
               }
            }
         }
