...
public abstract class AbstractMinecartEntity extends Entity implements net.minecraftforge.common.extensions.IForgeEntityMinecart {
   ...
   private static final Map<RailShape, Pair<Vec3i, Vec3i>> EXITS = Util.make(Maps.newEnumMap(RailShape.class), (p_226574_0_) -> {
   ...
   public AxisAlignedBB func_70114_g(Entity p_70114_1_) {
      ...
      return p_70114_1_.isPushable() ? p_70114_1_.getBoundingBox() : null;
   }
   ...
   private static Pair<Vec3i, Vec3i> exits(RailShape p_226573_0_) {
   ...
   public void tick() {
      ...
      if (this.level.isClientSide) {
         ...
         if (canUseRail() && blockstate.is(BlockTags.RAILS)) {
   ...
   protected void comeOffTrack() {
      ...
      if (getMaxSpeedAirVertical() > 0 && getDeltaMovement().y > getMaxSpeedAirVertical()) {
          if(Math.abs(getDeltaMovement().x) < 0.3f && Math.abs(getDeltaMovement().z) < 0.3f)
              setDeltaMovement(new Vec3d(getDeltaMovement().x, 0.15f, getDeltaMovement().z));
          else
              setDeltaMovement(new Vec3d(getDeltaMovement().x, getMaxSpeedAirVertical(), getDeltaMovement().z));
      }
   ...
   protected void moveAlongTrack(BlockPos p_180460_1_, BlockState p_180460_2_) {
      ...
      switch(railshape) {
      case ASCENDING_EAST:
         this.setDeltaMovement(vec3d1.add(-1*getSlopeAdjustment(), 0.0D, 0.0D));
         ++d1;
         break;
      case ASCENDING_WEST:
         this.setDeltaMovement(vec3d1.add(getSlopeAdjustment(), 0.0D, 0.0D));
         ++d1;
         break;
      case ASCENDING_NORTH:
         this.setDeltaMovement(vec3d1.add(0.0D, 0.0D, getSlopeAdjustment()));
         ++d1;
         break;
      case ASCENDING_SOUTH:
         this.setDeltaMovement(vec3d1.add(0.0D, 0.0D, -1*getSlopeAdjustment()));
         ++d1;
      }
      ...
      double d23 = (double)p_180460_1_.getX() + 0.5D + (double)vec3i.getX() * 0.5D;
      ...
      if (vec3i.getY() != 0 && MathHelper.floor(this.getX()) - p_180460_1_.getX() == vec3i.getX() && MathHelper.floor(this.getZ()) - p_180460_1_.getZ() == vec3i.getZ()) {
         this.setPos(this.getX(), this.getY() + (double)vec3i.getY(), this.getZ());
      } else if (vec3i1.getY() != 0 && MathHelper.floor(this.getX()) - p_180460_1_.getX() == vec3i1.getX() && MathHelper.floor(this.getZ()) - p_180460_1_.getZ() == vec3i1.getZ()) {
      ...
      if (vec3d3 != null && vec3d != null) {
         ...
         double d18 = Math.sqrt(getHorizontalDistanceSqr(vec3d4));
         if (d18 > 0.0D) {
            this.setDeltaMovement(vec3d4.multiply((d18 + d17) / d18, 1.0D, (d18 + d17) / d18));
         }
      ...
      if (j != p_180460_1_.getX() || i != p_180460_1_.getZ()) {
         ...
         double d26 = Math.sqrt(getHorizontalDistanceSqr(vec3d5));
         this.setDeltaMovement(d26 * (double)(j - p_180460_1_.getX()), vec3d5.y, d26 * (double)(i - p_180460_1_.getZ()));
      }
      ...
      if (flag && shouldDoRailFunctions()) {
         ...
         double d27 = Math.sqrt(getHorizontalDistanceSqr(vec3d6));
         if (d27 > 0.01D) {
            double d19 = 0.06D;
            this.setDeltaMovement(vec3d6.add(vec3d6.x / d27 * 0.06D, 0.0D, vec3d6.z / d27 * 0.06D));
         } else {
            ...
            this.setDeltaMovement(d20, vec3d7.y, d21);
         }
      }
   ...
   @Nullable
   @OnlyIn(Dist.CLIENT)
   public Vec3d getPosOffs(double p_70495_1_, double p_70495_3_, double p_70495_5_, double p_70495_7_) {
            ...
            if (!this.hasPassenger(p_70108_1_)) {
               ...
               if (d2 >= (double)1.0E-4F) {
                  ...
                  if (p_70108_1_ instanceof AbstractMinecartEntity) {
                     ...
                     Vec3d vec3d1 = (new Vec3d((double)MathHelper.cos(this.yRot * ((float)Math.PI / 180F)), 0.0D, (double)MathHelper.sin(this.yRot * ((float)Math.PI / 180F)))).normalize();
                     ...
                     Vec3d vec3d3 = p_70108_1_.getDeltaMovement();
                     if (((AbstractMinecartEntity)p_70108_1_).isPoweredCart() && !this.isPoweredCart()) {
                        ...
                        p_70108_1_.setDeltaMovement(vec3d3.multiply(0.95D, 1.0D, 0.95D));
                     } else if (!((AbstractMinecartEntity)p_70108_1_).isPoweredCart() && this.isPoweredCart()) {
                        p_70108_1_.setDeltaMovement(vec3d3.multiply(0.2D, 1.0D, 0.2D));
                        p_70108_1_.push(vec3d2.x + d0, 0.0D, vec3d2.z + d1);
   ...
   public void moveMinecartOnRail(BlockPos pos) { //Non-default because getMaximumSpeed is protected
      ...
      Vec3d vec3d1 = mc.getDeltaMovement();
