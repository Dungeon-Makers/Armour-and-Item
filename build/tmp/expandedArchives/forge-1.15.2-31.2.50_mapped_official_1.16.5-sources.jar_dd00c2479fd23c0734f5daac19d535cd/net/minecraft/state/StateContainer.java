...
public class StateContainer<O, S extends IStateHolder<S>> {
   ...
   private final ImmutableSortedMap<String, IProperty<?>> propertiesByName;
   ...
   protected <A extends StateHolder<O, S>> StateContainer(O p_i49005_1_, StateContainer.IFactory<O, S, A> p_i49005_2_, Map<String, IProperty<?>> p_i49005_3_) {
      ...
      this.propertiesByName = ImmutableSortedMap.copyOf(p_i49005_3_);
      ...
      for(IProperty<?> iproperty : this.propertiesByName.values()) {
         stream = stream.flatMap((p_200999_1_) -> {
            return iproperty.getPossibleValues().stream().map((p_200998_1_) -> {
      ...
      stream.forEach((p_201000_5_) -> {
         Map<IProperty<?>, Comparable<?>> map1 = MapPopulator.func_179400_b(this.propertiesByName.values(), p_201000_5_);
   ...
   public String toString() {
      return MoreObjects.toStringHelper(this).add("block", this.owner).add("properties", this.propertiesByName.values().stream().map(IProperty::getName).collect(Collectors.toList())).toString();
   }
   ...
   public static class Builder<O, S extends IStateHolder<S>> {
      ...
      public <A extends StateHolder<O, S>> StateContainer<O, S> func_206893_a(StateContainer.IFactory<O, S, A> p_206893_1_) {
