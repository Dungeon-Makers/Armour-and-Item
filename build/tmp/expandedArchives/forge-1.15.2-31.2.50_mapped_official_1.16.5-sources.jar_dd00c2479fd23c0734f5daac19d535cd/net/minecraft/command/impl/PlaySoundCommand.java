      ...
      while(true) {
         ...
         while(true) {
            ...
            if (!(p_198573_7_ <= 0.0F)) {
               ...
               vec3d = new Vec3d(serverplayerentity.getX() + d1 / d5 * 2.0D, serverplayerentity.getY() + d2 / d5 * 2.0D, serverplayerentity.getZ() + d3 / d5 * 2.0D);
               f = p_198573_7_;
               break;
            }
         }

         serverplayerentity.connection.send(new SPlaySoundPacket(p_198573_2_, p_198573_3_, vec3d, f, p_198573_6_));
         ++i;
      }
   }
}