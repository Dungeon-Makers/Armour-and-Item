...
public final class ItemStack extends net.minecraftforge.common.capabilities.CapabilityProvider<ItemStack> implements net.minecraftforge.common.extensions.IForgeItemStack {
   ...
   public static final DecimalFormat ATTRIBUTE_MODIFIER_FORMAT = func_208306_D();
   private int count;
   private int popTime;
   @Deprecated
   ...
   public ActionResultType useOn(ItemUseContext p_196084_1_) {
      if (!p_196084_1_.level.isClientSide) return net.minecraftforge.common.ForgeHooks.onPlaceItemIntoWorld(p_196084_1_);
   ...
   public boolean isDamageableItem() {
      if (!this.emptyCacheFlag && this.getItem().getMaxDamage(this) > 0) {
   ...
   public ITextComponent getHoverName() {
      ...
      if (compoundnbt != null && compoundnbt.contains("Name", 8)) {
         try {
            ITextComponent itextcomponent = ITextComponent.Serializer.func_150699_a(compoundnbt.getString("Name"));
   ...
   public List<ITextComponent> getTooltipLines(@Nullable PlayerEntity p_82840_1_, ITooltipFlag p_82840_2_) {
      ...
      ITextComponent itextcomponent = (new StringTextComponent("")).func_150257_a(this.getHoverName()).func_211708_a(this.getRarity().color);
      ...
      if (!p_82840_2_.isAdvanced() && !this.hasCustomHoverName() && this.getItem() == Items.FILLED_MAP) {
         list.add((new StringTextComponent("#" + FilledMapItem.getMapId(this))).func_211708_a(TextFormatting.GRAY));
      }
         ...
         if (this.tag.contains("display", 10)) {
            ...
            if (compoundnbt.contains("color", 3)) {
               if (p_82840_2_.isAdvanced()) {
                  list.add((new TranslationTextComponent("item.color", String.format("#%06X", compoundnbt.getInt("color")))).func_211708_a(TextFormatting.GRAY));
               } else {
                  list.add((new TranslationTextComponent("item.dyed")).func_211709_a(new TextFormatting[]{TextFormatting.GRAY, TextFormatting.ITALIC}));
               }
            }
            ...
            if (compoundnbt.getTagType("Lore") == 9) {
               ...
               for(int j = 0; j < listnbt.size(); ++j) {
                  ...
                  try {
                     ITextComponent itextcomponent1 = ITextComponent.Serializer.func_150699_a(s);
                     if (itextcomponent1 != null) {
                        list.add(TextComponentUtils.func_211401_a(itextcomponent1, (new Style()).func_150238_a(TextFormatting.DARK_PURPLE).func_150217_b(true)));
                     }
      ...
      for(EquipmentSlotType equipmentslottype : EquipmentSlotType.values()) {
         ...
         if (!multimap.isEmpty() && (i & 2) == 0) {
            ...
            list.add((new TranslationTextComponent("item.modifiers." + equipmentslottype.getName())).func_211708_a(TextFormatting.GRAY));
            ...
            for(Entry<String, AttributeModifier> entry : multimap.entries()) {
               ...
               if (p_82840_1_ != null) {
                  if (attributemodifier.getId() == Item.BASE_ATTACK_DAMAGE_UUID) {
                     d0 = d0 + p_82840_1_.getAttribute(SharedMonsterAttributes.field_111264_e).getBaseValue();
                     ...
                     d0 += p_82840_1_.getAttribute(SharedMonsterAttributes.field_188790_f).getBaseValue();
                     flag = true;
                  }
               }
               ...
               if (flag) {
                  list.add((new StringTextComponent(" ")).func_150257_a(new TranslationTextComponent("attribute.modifier.equals." + attributemodifier.getOperation().toValue(), ATTRIBUTE_MODIFIER_FORMAT.format(d1), new TranslationTextComponent("attribute.name." + (String)entry.getKey()))).func_211708_a(TextFormatting.DARK_GREEN));
               } else if (d0 > 0.0D) {
                  list.add((new TranslationTextComponent("attribute.modifier.plus." + attributemodifier.getOperation().toValue(), ATTRIBUTE_MODIFIER_FORMAT.format(d1), new TranslationTextComponent("attribute.name." + (String)entry.getKey()))).func_211708_a(TextFormatting.BLUE));
               } else if (d0 < 0.0D) {
                  d1 = d1 * -1.0D;
                  list.add((new TranslationTextComponent("attribute.modifier.take." + attributemodifier.getOperation().toValue(), ATTRIBUTE_MODIFIER_FORMAT.format(d1), new TranslationTextComponent("attribute.name." + (String)entry.getKey()))).func_211708_a(TextFormatting.RED));
               }
            }
         }
      }
      ...
      if (this.hasTag() && this.getTag().getBoolean("Unbreakable") && (i & 4) == 0) {
         list.add((new TranslationTextComponent("item.unbreakable")).func_211708_a(TextFormatting.BLUE));
      }
      ...
      if (this.hasTag() && this.tag.contains("CanDestroy", 9) && (i & 8) == 0) {
         ...
         if (!listnbt1.isEmpty()) {
            list.add(new StringTextComponent(""));
            list.add((new TranslationTextComponent("item.canBreak")).func_211708_a(TextFormatting.GRAY));
      ...
      if (this.hasTag() && this.tag.contains("CanPlaceOn", 9) && (i & 16) == 0) {
         ...
         if (!listnbt2.isEmpty()) {
            list.add(new StringTextComponent(""));
            list.add((new TranslationTextComponent("item.canPlace")).func_211708_a(TextFormatting.GRAY));
      ...
      if (p_82840_2_.isAdvanced()) {
         ...
         list.add((new StringTextComponent(Registry.ITEM.getKey(this.getItem()).toString())).func_211708_a(TextFormatting.DARK_GRAY));
         if (this.hasTag()) {
            list.add((new TranslationTextComponent("item.nbt_tags", this.getTag().getAllKeys().size())).func_211708_a(TextFormatting.DARK_GRAY));
         }
      }
   ...
   public static void appendEnchantmentNames(List<ITextComponent> p_222120_0_, ListNBT p_222120_1_) {
      for(int i = 0; i < p_222120_1_.size(); ++i) {
         ...
         Registry.ENCHANTMENT.func_218349_b(ResourceLocation.tryParse(compoundnbt.getString("id"))).ifPresent((p_222123_2_) -> {
   ...
   private static Collection<ITextComponent> expandBlockState(String p_206845_0_) {
      try {
         ...
         if (flag || flag1) {
            if (flag) {
               return Lists.newArrayList(blockstate.getBlock().func_200291_n().func_211708_a(TextFormatting.DARK_GRAY));
            }

            Tag<Block> tag = BlockTags.getAllTags().getTag(resourcelocation);
            if (tag != null) {
               Collection<Block> collection = tag.func_199885_a();
               if (!collection.isEmpty()) {
                  return collection.stream().map(Block::func_200291_n).map((p_222119_0_) -> {
                     return p_222119_0_.func_211708_a(TextFormatting.DARK_GRAY);
   ...
   public void func_82842_a(@Nullable ItemFrameEntity p_82842_1_) {
      this.field_82843_f = p_82842_1_;
   }

   @Nullable
   public ItemFrameEntity getFrame() {
      return this.emptyCacheFlag ? null : this.field_82843_f;
   }
   ...
   public Multimap<String, AttributeModifier> getAttributeModifiers(EquipmentSlotType p_111283_1_) {
      ...
      multimap.values().forEach((p_226631_0_) -> {
         p_226631_0_.func_111168_a(false);
      });
      return multimap;
   }
   ...
   public void addAttributeModifier(String p_185129_1_, AttributeModifier p_185129_2_, @Nullable EquipmentSlotType p_185129_3_) {
      ...
      ListNBT listnbt = this.tag.getList("AttributeModifiers", 10);
      CompoundNBT compoundnbt = SharedMonsterAttributes.func_111262_a(p_185129_2_);
   ...
   public ITextComponent getDisplayName() {
      ITextComponent itextcomponent = (new StringTextComponent("")).func_150257_a(this.getHoverName());
      if (this.hasCustomHoverName()) {
         itextcomponent.func_211708_a(TextFormatting.ITALIC);
      }

      ITextComponent itextcomponent1 = TextComponentUtils.func_197676_a(itextcomponent);
      if (!this.emptyCacheFlag) {
         CompoundNBT compoundnbt = this.save(new CompoundNBT());
         itextcomponent1.func_211708_a(this.getRarity().color).func_211710_a((p_226632_1_) -> {
            p_226632_1_.func_150209_a(new HoverEvent(HoverEvent.Action.SHOW_ITEM, new StringTextComponent(compoundnbt.toString())));
         });
      }
   ...
   public boolean hasAdventureModeBreakTagForBlock(NetworkTagManager p_206848_1_, CachedBlockInfo p_206848_2_) {
   ...
   public boolean hasAdventureModePlaceTagForBlock(NetworkTagManager p_206847_1_, CachedBlockInfo p_206847_2_) {
