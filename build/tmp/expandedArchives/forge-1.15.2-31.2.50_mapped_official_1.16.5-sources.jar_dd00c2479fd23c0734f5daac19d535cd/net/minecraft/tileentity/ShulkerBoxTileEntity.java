...
public class ShulkerBoxTileEntity extends LockableLootTileEntity implements ISidedInventory, ITickableTileEntity {
         ...
         if (!list.isEmpty()) {
            for(int i = 0; i < list.size(); ++i) {
               ...
               if (entity.getPistonPushReaction() != PushReaction.IGNORE) {
                  ...
                  entity.move(MoverType.SHULKER_BOX, new Vec3d(d0 * (double)direction.getStepX(), d1 * (double)direction.getStepY(), d2 * (double)direction.getStepZ()));
               }
            }
   ...
   private void doNeighborUpdates() {
      this.getBlockState().func_196946_a(this.getLevel(), this.getBlockPos(), 3);
   }
