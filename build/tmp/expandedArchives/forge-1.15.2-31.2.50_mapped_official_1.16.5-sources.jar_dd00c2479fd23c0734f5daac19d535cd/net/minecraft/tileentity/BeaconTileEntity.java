...
public class BeaconTileEntity extends TileEntity implements INamedContainerProvider, ITickableTileEntity {
   ...
   private static final Set<Effect> VALID_EFFECTS = Arrays.stream(BEACON_EFFECTS).<Effect>flatMap(Arrays::stream).collect(Collectors.toSet());
      ...
      for(int i = 1; i <= 4; this.levels = i++) {
         ...
         for(int k = p_213927_1_ - i; k <= p_213927_1_ + i && flag; ++k) {
            for(int l = p_213927_3_ - i; l <= p_213927_3_ + i; ++l) {
               if (!this.level.getBlockState(new BlockPos(k, j, l)).isBeaconBase(this.level, new BlockPos(k, j, l), getBlockPos())) {
                  flag = false;
                  break;
               }
            }
   ...
   public void func_145839_a(CompoundNBT p_145839_1_) {
      ...
      this.primaryPower = getValidEffectById(p_145839_1_.getInt("Primary"));
      this.secondaryPower = getValidEffectById(p_145839_1_.getInt("Secondary"));
      if (p_145839_1_.contains("CustomName", 8)) {
         this.name = ITextComponent.Serializer.func_150699_a(p_145839_1_.getString("CustomName"));
      }

      this.lockKey = LockCode.fromTag(p_145839_1_);
   }
