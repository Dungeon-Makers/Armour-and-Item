   ...
   private Path findPath(PathPoint p_227479_1_, Map<FlaggedPathPoint, BlockPos> p_227479_2_, float p_227479_3_, int p_227479_4_, float p_227479_5_) {
      ...
      while(!this.openSet.isEmpty()) {
         ...
         set.stream().filter((p_224781_2_) -> {
            return pathpoint.distanceManhattan(p_224781_2_) <= (float)p_227479_4_;
         }).forEach(FlaggedPathPoint::setReached);
         if (set.stream().anyMatch(FlaggedPathPoint::isReached)) {
            break;
         }
      ...
      Stream<Path> stream;
      if (set.stream().anyMatch(FlaggedPathPoint::isReached)) {
         stream = set.stream().filter(FlaggedPathPoint::isReached).map((p_224778_2_) -> {
            return this.reconstructPath(p_224778_2_.getBestNode(), p_227479_2_.get(p_224778_2_), true);
         }).sorted(Comparator.comparingInt(Path::getNodeCount));
      } else {
