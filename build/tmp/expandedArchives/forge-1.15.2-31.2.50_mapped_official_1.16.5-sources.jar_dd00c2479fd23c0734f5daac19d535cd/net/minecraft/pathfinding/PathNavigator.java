...
public abstract class PathNavigator {
   ...
   public PathNavigator(MobEntity p_i1671_1_, World p_i1671_2_) {
      ...
      this.field_226333_p_ = p_i1671_1_.getAttribute(SharedMonsterAttributes.field_111265_b);
   ...
   public Path createPath(Entity p_75494_1_, int p_75494_2_) {
      return this.createPath(ImmutableSet.of(new BlockPos(p_75494_1_)), 16, true, p_75494_2_);
   }
   ...
   protected Path createPath(Set<BlockPos> p_225464_1_, int p_225464_2_, boolean p_225464_3_, int p_225464_4_) {
      if (p_225464_1_.isEmpty()) {
         ...
         BlockPos blockpos = p_225464_3_ ? (new BlockPos(this.mob)).above() : new BlockPos(this.mob);
   ...
   public void tick() {
      ...
      if (!this.isDone()) {
         if (this.canUpdatePath()) {
            ...
            Vec3d vec3d = this.getTempMobPos();
            Vec3d vec3d1 = this.path.getEntityPosAtNode(this.mob, this.path.getNextNodeIndex());
         ...
         if (!this.isDone()) {
            ...
            this.mob.getMoveControl().setWantedPosition(vec3d2.x, this.level.getBlockState(blockpos.below()).isAir() ? vec3d2.y : WalkNodeProcessor.getFloorLevel(this.level, blockpos), vec3d2.z, this.speedModifier);
         }
      }
   }
   ...
   protected void followThePath() {
      ...
      Vec3d vec3d1 = this.path.func_186310_f();
      // Forge: fix MC-94054
      if (Math.abs(this.mob.getX() - (vec3d1.x + ((int)(this.mob.getBbWidth() + 1) / 2D))) < (double)this.maxDistanceToWaypoint && Math.abs(this.mob.getZ() - (vec3d1.z + ((int)(this.mob.getBbWidth() + 1) / 2D))) < (double)this.maxDistanceToWaypoint && Math.abs(this.mob.getY() - vec3d1.y) < 1.0D) {
         this.path.setNextNodeIndex(this.path.getNextNodeIndex() + 1);
      }
   ...
   protected abstract boolean canMoveDirectly(Vec3d p_75493_1_, Vec3d p_75493_2_, int p_75493_3_, int p_75493_4_, int p_75493_5_);
   ...
   public void recomputePath(BlockPos p_220970_1_) {
      if (this.path != null && !this.path.isDone() && this.path.getNodeCount() != 0) {
         ...
         if (p_220970_1_.closerThan(vec3d, (double)(this.path.getNodeCount() - this.path.getNextNodeIndex()))) {
            this.recomputePath();
         }
