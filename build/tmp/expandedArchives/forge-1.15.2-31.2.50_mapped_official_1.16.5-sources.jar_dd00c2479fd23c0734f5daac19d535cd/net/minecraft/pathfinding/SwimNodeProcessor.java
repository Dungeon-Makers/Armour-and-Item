...
public class SwimNodeProcessor extends NodeProcessor {
   ...
   public PathNodeType getBlockPathType(IBlockReader p_186330_1_, int p_186330_2_, int p_186330_3_, int p_186330_4_) {
      ...
      if (ifluidstate.isEmpty() && blockstate.isPathfindable(p_186330_1_, blockpos.below(), PathType.WATER) && blockstate.isAir()) {
         return PathNodeType.BREACH;
      } else {
         return ifluidstate.is(FluidTags.WATER) && blockstate.isPathfindable(p_186330_1_, blockpos, PathType.WATER) ? PathNodeType.WATER : PathNodeType.BLOCKED;
      }
   }
      ...
      for(int i = p_186327_1_; i < p_186327_1_ + this.entityWidth; ++i) {
         for(int j = p_186327_2_; j < p_186327_2_ + this.entityHeight; ++j) {
            for(int k = p_186327_3_; k < p_186327_3_ + this.entityDepth; ++k) {
               IFluidState ifluidstate = this.level.getFluidState(blockpos$mutable.set(i, j, k));
               BlockState blockstate = this.level.getBlockState(blockpos$mutable.set(i, j, k));
               if (ifluidstate.isEmpty() && blockstate.isPathfindable(this.level, blockpos$mutable.below(), PathType.WATER) && blockstate.isAir()) {
                  return PathNodeType.BREACH;
               }
