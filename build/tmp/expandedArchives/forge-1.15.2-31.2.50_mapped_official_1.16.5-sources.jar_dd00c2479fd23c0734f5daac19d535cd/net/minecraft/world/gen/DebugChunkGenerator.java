...
public class DebugChunkGenerator extends ChunkGenerator<DebugGenerationSettings> {
   private static final List<BlockState> ALL_BLOCKS = StreamSupport.stream(Registry.BLOCK.spliterator(), false).flatMap((p_199812_0_) -> {
      return p_199812_0_.getStateDefinition().getPossibleStates().stream();
   ...
   public void func_202092_b(WorldGenRegion p_202092_1_) {
      ...
      for(int k = 0; k < 16; ++k) {
         for(int l = 0; l < 16; ++l) {
            int i1 = (i << 4) + k;
            int j1 = (j << 4) + l;
            p_202092_1_.setBlock(blockpos$mutable.set(i1, 60, j1), BARRIER, 2);
            BlockState blockstate = getBlockStateFor(i1, j1);
            if (blockstate != null) {
               p_202092_1_.setBlock(blockpos$mutable.set(i1, 70, j1), blockstate, 2);
            }
         }
      }
