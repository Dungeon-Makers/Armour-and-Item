...
public interface IEntityReader {
   ...
   default boolean isUnobstructed(@Nullable Entity p_195585_1_, VoxelShape p_195585_2_) {
      return p_195585_2_.isEmpty() ? true : this.getEntities(p_195585_1_, p_195585_2_.bounds()).stream().filter((p_217364_1_) -> {
         return !p_217364_1_.removed && p_217364_1_.blocksBuilding && (p_195585_1_ == null || !p_217364_1_.isPassengerOfSameVehicle(p_195585_1_));
      }).noneMatch((p_217356_1_) -> {
         return VoxelShapes.joinIsNotEmpty(p_195585_2_, VoxelShapes.create(p_217356_1_.getBoundingBox()), IBooleanFunction.AND);
      });
   }
   ...
   default Stream<VoxelShape> func_223439_a(@Nullable Entity p_223439_1_, AxisAlignedBB p_223439_2_, Set<Entity> p_223439_3_) {
      if (p_223439_2_.getSize() < 1.0E-7D) {
         return Stream.empty();
      } else {
         AxisAlignedBB axisalignedbb = p_223439_2_.inflate(1.0E-7D);
         return this.getEntities(p_223439_1_, axisalignedbb).stream().filter((p_217367_1_) -> {
            return !p_223439_3_.contains(p_217367_1_);
         }).filter((p_223442_1_) -> {
            return p_223439_1_ == null || !p_223439_1_.isPassengerOfSameVehicle(p_223442_1_);
         }).flatMap((p_217368_1_) -> {
            return Stream.of(p_217368_1_.func_70046_E(), p_223439_1_ == null ? null : p_223439_1_.func_70114_g(p_217368_1_));
   ...
   default PlayerEntity func_217365_a(double p_217365_1_, double p_217365_3_, double p_217365_5_) {
      ...
      for(PlayerEntity playerentity1 : this.players()) {
         if (EntityPredicates.NO_SPECTATORS.test(playerentity1)) {
            double d1 = playerentity1.distanceToSqr(p_217365_1_, playerentity1.getY(), p_217365_3_);
   ...
   default <T extends LivingEntity> T getNearestEntity(Class<? extends T> p_217360_1_, EntityPredicate p_217360_2_, @Nullable LivingEntity p_217360_3_, double p_217360_4_, double p_217360_6_, double p_217360_8_, AxisAlignedBB p_217360_10_) {
      return this.getNearestEntity(this.getEntitiesOfClass(p_217360_1_, p_217360_10_, (Predicate<T>)null), p_217360_2_, p_217360_3_, p_217360_4_, p_217360_6_, p_217360_8_);
   }
   ...
   default <T extends LivingEntity> T getNearestLoadedEntity(Class<? extends T> p_225318_1_, EntityPredicate p_225318_2_, @Nullable LivingEntity p_225318_3_, double p_225318_4_, double p_225318_6_, double p_225318_8_, AxisAlignedBB p_225318_10_) {
      return this.getNearestEntity(this.getLoadedEntitiesOfClass(p_225318_1_, p_225318_10_, (Predicate<T>)null), p_225318_2_, p_225318_3_, p_225318_4_, p_225318_6_, p_225318_8_);
   }
   ...
   default <T extends LivingEntity> List<T> getNearbyEntities(Class<? extends T> p_217374_1_, EntityPredicate p_217374_2_, LivingEntity p_217374_3_, AxisAlignedBB p_217374_4_) {
      List<T> list = this.getEntitiesOfClass(p_217374_1_, p_217374_4_, (Predicate<T>)null);
