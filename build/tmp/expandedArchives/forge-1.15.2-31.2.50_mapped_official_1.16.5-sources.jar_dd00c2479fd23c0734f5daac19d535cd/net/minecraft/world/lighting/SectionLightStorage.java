...
public abstract class SectionLightStorage<M extends LightDataMap<M>> extends SectionDistanceGraph {
   ...
   protected void markNewInconsistencies(LightEngine<M, ?> p_215522_1_, boolean p_215522_2_, boolean p_215522_3_) {
      if (this.hasInconsistencies() || !this.queuedSections.isEmpty()) {
         ...
         for(long i2 : this.toRemove) {
            this.onNodeRemoved(i2);
         }
         ...
         this.updatingSectionData.clearCache();
         if (!p_215522_3_) {
            for(long j2 : this.queuedSections.keySet()) {
               if (this.storingLightForSection(j2)) {
                  int l2 = SectionPos.sectionToBlockCoord(SectionPos.x(j2));
                  int i3 = SectionPos.sectionToBlockCoord(SectionPos.y(j2));
                  int k = SectionPos.sectionToBlockCoord(SectionPos.z(j2));

                  for(Direction direction : DIRECTIONS) {
                     long l = SectionPos.offset(j2, direction);
                     if (!this.queuedSections.containsKey(l) && this.storingLightForSection(l)) {
                        for(int i1 = 0; i1 < 16; ++i1) {
                           for(int j1 = 0; j1 < 16; ++j1) {
                              ...
                              switch(direction) {
                              case DOWN:
                                 k1 = BlockPos.asLong(l2 + j1, i3, k + i1);
                                 l1 = BlockPos.asLong(l2 + j1, i3 - 1, k + i1);
                                 break;
                              case UP:
                                 k1 = BlockPos.asLong(l2 + j1, i3 + 16 - 1, k + i1);
                                 l1 = BlockPos.asLong(l2 + j1, i3 + 16, k + i1);
                                 break;
                              case NORTH:
                                 k1 = BlockPos.asLong(l2 + i1, i3 + j1, k);
                                 l1 = BlockPos.asLong(l2 + i1, i3 + j1, k - 1);
                                 break;
                              case SOUTH:
                                 k1 = BlockPos.asLong(l2 + i1, i3 + j1, k + 16 - 1);
                                 l1 = BlockPos.asLong(l2 + i1, i3 + j1, k + 16);
                                 break;
                              case WEST:
                                 k1 = BlockPos.asLong(l2, i3 + i1, k + j1);
                                 l1 = BlockPos.asLong(l2 - 1, i3 + i1, k + j1);
                                 break;
                              default:
                                 k1 = BlockPos.asLong(l2 + 16 - 1, i3 + i1, k + j1);
                                 l1 = BlockPos.asLong(l2 + 16, i3 + i1, k + j1);
                              }

                              p_215522_1_.checkEdge(k1, l1, p_215522_1_.computeLevelFromNeighbor(k1, l1, p_215522_1_.getLevel(k1)), false);
                              p_215522_1_.checkEdge(l1, k1, p_215522_1_.computeLevelFromNeighbor(l1, k1, p_215522_1_.getLevel(l1)), false);
                           }
                        }
                     }
                  }
   ...
   protected void queueSectionData(long p_215529_1_, @Nullable NibbleArray p_215529_3_) {
      if (p_215529_3_ != null) {
