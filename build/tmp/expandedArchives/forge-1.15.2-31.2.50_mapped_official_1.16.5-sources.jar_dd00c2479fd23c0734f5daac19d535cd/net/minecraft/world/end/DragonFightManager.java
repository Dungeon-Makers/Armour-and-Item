...
public class DragonFightManager {
   ...
   public DragonFightManager(ServerWorld p_i46669_1_, CompoundNBT p_i46669_2_, net.minecraft.world.dimension.EndDimension dim) {
      this.level = p_i46669_1_;
      if (p_i46669_2_.contains("DragonKilled", 99)) {
         if (p_i46669_2_.hasUUID("DragonUUID")) {
            this.dragonUUID = p_i46669_2_.getUUID("DragonUUID");
         }

         this.dragonKilled = p_i46669_2_.getBoolean("DragonKilled");
         this.previouslyKilled = p_i46669_2_.getBoolean("PreviouslyKilled");
         this.needsStateScanning = !p_i46669_2_.getBoolean("LegacyScanPerformed"); // Forge: fix MC-105080
         if (p_i46669_2_.getBoolean("IsRespawning")) {
            this.respawnStage = DragonSpawnState.START;
         }

         if (p_i46669_2_.contains("ExitPortalLocation", 10)) {
            this.portalLocation = NBTUtil.readBlockPos(p_i46669_2_.getCompound("ExitPortalLocation"));
         }
      } else {
      ...
      if (p_i46669_2_.contains("Gateways", 9)) {
         ListNBT listnbt = p_i46669_2_.getList("Gateways", 3);
   ...
   public CompoundNBT saveData() {
      ...
      if (this.dragonUUID != null) {
         compoundnbt.putUUID("DragonUUID", this.dragonUUID);
      }
   ...
   private void updateCrystalCount() {
      ...
      for(EndSpikeFeature.EndSpike endspikefeature$endspike : EndSpikeFeature.func_214554_a(this.level)) {
   ...
   private void spawnNewGateway(BlockPos p_186089_1_) {
      ...
      Feature.END_GATEWAY.configured(EndGatewayConfig.delayedExitSearch()).func_222734_a(this.level, this.level.getChunkSource().getGenerator(), new Random(), p_186089_1_);
   }

   private void spawnExitPortal(boolean p_186094_1_) {
      EndPodiumFeature endpodiumfeature = new EndPodiumFeature(p_186094_1_);
      if (this.portalLocation == null) {
         for(this.portalLocation = this.level.getHeightmapPos(Heightmap.Type.MOTION_BLOCKING_NO_LEAVES, EndPodiumFeature.END_PODIUM_LOCATION).below(); this.level.getBlockState(this.portalLocation).getBlock() == Blocks.BEDROCK && this.portalLocation.getY() > this.level.getSeaLevel(); this.portalLocation = this.portalLocation.below()) {
            ;
         }
      }

      endpodiumfeature.configured(IFeatureConfig.NONE).func_222734_a(this.level, this.level.getChunkSource().getGenerator(), new Random(), this.portalLocation);
   }
            ...
            for(int i = 0; i < this.exitPortalPattern.getWidth(); ++i) {
               for(int j = 0; j < this.exitPortalPattern.getHeight(); ++j) {
                  for(int k = 0; k < this.exitPortalPattern.getDepth(); ++k) {
                     ...
                     if (cachedblockinfo.getState().getBlock() == Blocks.BEDROCK || cachedblockinfo.getState().getBlock() == Blocks.END_PORTAL) {
