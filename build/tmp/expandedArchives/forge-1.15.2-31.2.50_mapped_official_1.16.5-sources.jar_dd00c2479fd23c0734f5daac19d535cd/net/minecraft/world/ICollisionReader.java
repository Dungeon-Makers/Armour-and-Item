...
public interface ICollisionReader extends IBlockReader {
   ...
   default Stream<VoxelShape> func_226667_c_(@Nullable Entity p_226667_1_, AxisAlignedBB p_226667_2_, Set<Entity> p_226667_3_) {
      return Streams.concat(this.getBlockCollisions(p_226667_1_, p_226667_2_), this.func_223439_a(p_226667_1_, p_226667_2_, p_226667_3_));
   }

   default Stream<VoxelShape> getBlockCollisions(@Nullable final Entity p_226666_1_, AxisAlignedBB p_226666_2_) {
      ...
      final ISelectionContext iselectioncontext = p_226666_1_ == null ? ISelectionContext.empty() : ISelectionContext.of(p_226666_1_);
      ...
      return StreamSupport.stream(new AbstractSpliterator<VoxelShape>(Long.MAX_VALUE, 1280) {
         ...
         public boolean tryAdvance(Consumer<? super VoxelShape> p_tryAdvance_1_) {
            if (!this.field_226670_a_) {
               ...
               VoxelShape voxelshape1 = ICollisionReader.this.getWorldBorder().getCollisionShape();
               boolean flag = VoxelShapes.joinIsNotEmpty(voxelshape1, VoxelShapes.create(p_226666_1_.getBoundingBox().deflate(1.0E-7D)), IBooleanFunction.AND);
               boolean flag1 = VoxelShapes.joinIsNotEmpty(voxelshape1, VoxelShapes.create(p_226666_1_.getBoundingBox().inflate(1.0E-7D)), IBooleanFunction.AND);
               if (!flag && flag1) {
            ...
            while(true) {
               if (!cubecoordinateiterator.advance()) {
                  return false;
               }

               int j2 = cubecoordinateiterator.nextX();
               int k2 = cubecoordinateiterator.nextY();
               int l2 = cubecoordinateiterator.nextZ();
               int k1 = cubecoordinateiterator.getNextType();
               if (k1 != 3) {
                  int l1 = j2 >> 4;
                  int i2 = l2 >> 4;
                  IBlockReader iblockreader = ICollisionReader.this.getChunkForCollisions(l1, i2);
                  if (iblockreader != null) {
                     blockpos$mutable.set(j2, k2, l2);
                     BlockState blockstate = iblockreader.getBlockState(blockpos$mutable);
                     if ((k1 != 1 || blockstate.hasLargeCollisionShape()) && (k1 != 2 || blockstate.getBlock() == Blocks.MOVING_PISTON)) {
                        VoxelShape voxelshape2 = blockstate.getCollisionShape(ICollisionReader.this, blockpos$mutable, iselectioncontext);
                        voxelshape3 = voxelshape2.move((double)j2, (double)k2, (double)l2);
                        if (VoxelShapes.joinIsNotEmpty(voxelshape, voxelshape3, IBooleanFunction.AND)) {
                           break;
                        }
                     }
                  }
