...
public class EditSignScreen extends Screen {
   ...
   protected void init() {
      ...
      this.addButton(new Button(this.width / 2 - 100, this.height / 4 + 120, 200, 20, I18n.get("gui.done"), (p_214266_1_) -> {
         this.onDone();
      }));
      this.sign.setEditable(false);
      this.signField = new TextInputUtil(this.minecraft, () -> {
         return this.sign.getMessage(this.line).getString();
      }, (p_214265_1_) -> {
         this.sign.setMessage(this.line, new StringTextComponent(p_214265_1_));
      }, 90);
   }
   ...
   public void removed() {
      ...
      if (clientplaynethandler != null) {
         clientplaynethandler.send(new CUpdateSignPacket(this.sign.getBlockPos(), this.sign.getMessage(0), this.sign.getMessage(1), this.sign.getMessage(2), this.sign.getMessage(3)));
      }
   ...
   public boolean charTyped(char p_charTyped_1_, int p_charTyped_2_) {
      this.signField.charTyped(p_charTyped_1_);
      return true;
   }
   ...
   public boolean keyPressed(int p_keyPressed_1_, int p_keyPressed_2_, int p_keyPressed_3_) {
      if (p_keyPressed_1_ == 265) {
         ...
         this.signField.func_216899_b();
         return true;
      } else if (p_keyPressed_1_ != 264 && p_keyPressed_1_ != 257 && p_keyPressed_1_ != 335) {
         return this.signField.keyPressed(p_keyPressed_1_) ? true : super.keyPressed(p_keyPressed_1_, p_keyPressed_2_, p_keyPressed_3_);
      } else {
         this.line = this.line + 1 & 3;
         this.signField.func_216899_b();
         return true;
   ...
   public void render(int p_render_1_, int p_render_2_, float p_render_3_) {
      ...
      IVertexBuilder ivertexbuilder = material.buffer(irendertypebuffer$impl, this.signModel::renderType);
      ...
      for(int j = 0; j < astring.length; ++j) {
         astring[j] = this.sign.func_212364_a(j, (p_228192_1_) -> {
            List<ITextComponent> list = RenderComponentsUtil.func_178908_a(p_228192_1_, 90, this.minecraft.font, false, true);
            return list.isEmpty() ? "" : list.get(0).func_150254_d();
         });
      }

      Matrix4f matrix4f = matrixstack.last().pose();
      int k = this.signField.getCursorPos();
      int l = this.signField.getSelectionPos();
      int i1 = this.minecraft.font.isBidirectional() ? -1 : 1;
      int j1 = this.line * 10 - this.sign.messages.length * 5;

      for(int k1 = 0; k1 < astring.length; ++k1) {
         String s = astring[k1];
         if (s != null) {
            float f3 = (float)(-this.minecraft.font.width(s) / 2);
            this.minecraft.font.drawInBatch(s, f3, (float)(k1 * 10 - this.sign.messages.length * 5), i, false, matrix4f, irendertypebuffer$impl, false, 0, 15728880);
            if (k1 == this.line && k >= 0 && flag1) {
               int l1 = this.minecraft.font.width(s.substring(0, Math.max(Math.min(k, s.length()), 0)));
               int i2 = (l1 - this.minecraft.font.width(s) / 2) * i1;
               if (k >= s.length()) {
                  this.minecraft.font.drawInBatch("_", (float)i2, (float)j1, i, false, matrix4f, irendertypebuffer$impl, false, 0, 15728880);
               }
            }
         }
      }
      ...
      for(int k3 = 0; k3 < astring.length; ++k3) {
         ...
         if (s1 != null && k3 == this.line && k >= 0) {
            int l3 = this.minecraft.font.width(s1.substring(0, Math.max(Math.min(k, s1.length()), 0)));
            int i4 = (l3 - this.minecraft.font.width(s1) / 2) * i1;
            ...
            if (l != k) {
               int j4 = Math.min(k, l);
               int j2 = Math.max(k, l);
               int k2 = (this.minecraft.font.width(s1.substring(0, j4)) - this.minecraft.font.width(s1) / 2) * i1;
               int l2 = (this.minecraft.font.width(s1.substring(0, j2)) - this.minecraft.font.width(s1) / 2) * i1;
