...
public abstract class SlotGui extends FocusableGui implements IRenderable {
   ...
   protected void drawContainerBackground(Tessellator tessellator) {
      ...
      buffer.vertex((double)this.x0, (double)this.y1, 0.0D).uv(this.x0 / scale, (this.y1 + (int)this.yo) / scale).color(32, 32, 32, 255).endVertex();
      buffer.vertex((double)this.x1, (double)this.y1, 0.0D).uv(this.x1 / scale, (this.y1 + (int)this.yo) / scale).color(32, 32, 32, 255).endVertex();
      buffer.vertex((double)this.x1, (double)this.y0, 0.0D).uv(this.x1 / scale, (this.y0 + (int)this.yo) / scale).color(32, 32, 32, 255).endVertex();
      buffer.vertex((double)this.x0, (double)this.y0, 0.0D).uv(this.x0 / scale, (this.y0 + (int)this.yo) / scale).color(32, 32, 32, 255).endVertex();
      tessellator.end();
   }
}
