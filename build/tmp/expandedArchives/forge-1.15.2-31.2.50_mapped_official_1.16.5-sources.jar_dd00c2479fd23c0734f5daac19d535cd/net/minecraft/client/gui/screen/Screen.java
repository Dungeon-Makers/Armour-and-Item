...
public abstract class Screen extends FocusableGui implements IRenderable {
   ...
   public List<String> getTooltipFromItem(ItemStack p_getTooltipFromItem_1_) {
      List<ITextComponent> list = p_getTooltipFromItem_1_.getTooltipLines(this.minecraft.player, this.minecraft.options.advancedItemTooltips ? ITooltipFlag.TooltipFlags.ADVANCED : ITooltipFlag.TooltipFlags.NORMAL);
   ...
   public void renderTooltip(List<String> p_renderTooltip_1_, int p_renderTooltip_2_, int p_renderTooltip_3_, FontRenderer font) {
      ...
      if (false && !p_renderTooltip_1_.isEmpty()) {
         ...
         matrixstack.translate(0.0D, 0.0D, (double)this.itemRenderer.blitOffset);
         Matrix4f matrix4f = matrixstack.last().pose();

         for(int k1 = 0; k1 < p_renderTooltip_1_.size(); ++k1) {
            String s1 = p_renderTooltip_1_.get(k1);
            if (s1 != null) {
               this.font.drawInBatch(s1, (float)l1, (float)i2, -1, true, matrix4f, irendertypebuffer$impl, false, 0, 15728880);
            }
   ...
   protected void renderComponentHoverEffect(ITextComponent p_renderComponentHoverEffect_1_, int p_renderComponentHoverEffect_2_, int p_renderComponentHoverEffect_3_) {
      if (p_renderComponentHoverEffect_1_ != null && p_renderComponentHoverEffect_1_.getStyle().getHoverEvent() != null) {
         ...
         if (hoverevent.getAction() == HoverEvent.Action.SHOW_ITEM) {
            ...
            try {
               INBT inbt = JsonToNBT.parseTag(hoverevent.func_150702_b().getString());
               if (inbt instanceof CompoundNBT) {
                  itemstack = ItemStack.of((CompoundNBT)inbt);
               }
            ...
            if (this.minecraft.options.advancedItemTooltips) {
               try {
                  CompoundNBT compoundnbt = JsonToNBT.parseTag(hoverevent.func_150702_b().getString());
            ...
            this.renderTooltip(this.minecraft.font.func_78271_c(hoverevent.func_150702_b().func_150254_d(), Math.max(this.width / 2, 200)), p_renderComponentHoverEffect_2_, p_renderComponentHoverEffect_3_);
         }
   ...
   public void sendMessage(String p_sendMessage_1_, boolean p_sendMessage_2_) {
      ...
      if (p_sendMessage_2_) {
         this.minecraft.gui.getChat().addRecentChat(p_sendMessage_1_);
      }
   ...
   public void renderDirtBackground(int p_renderDirtBackground_1_) {
      ...
      bufferbuilder.vertex(0.0D, (double)this.height, 0.0D).uv(0.0F, (float)this.height / 32.0F + (float)p_renderDirtBackground_1_).color(64, 64, 64, 255).endVertex();
      bufferbuilder.vertex((double)this.width, (double)this.height, 0.0D).uv((float)this.width / 32.0F, (float)this.height / 32.0F + (float)p_renderDirtBackground_1_).color(64, 64, 64, 255).endVertex();
      bufferbuilder.vertex((double)this.width, 0.0D, 0.0D).uv((float)this.width / 32.0F, (float)p_renderDirtBackground_1_).color(64, 64, 64, 255).endVertex();
      bufferbuilder.vertex(0.0D, 0.0D, 0.0D).uv(0.0F, (float)p_renderDirtBackground_1_).color(64, 64, 64, 255).endVertex();
      tessellator.end();
      net.minecraftforge.common.MinecraftForge.EVENT_BUS.post(new net.minecraftforge.client.event.GuiScreenEvent.BackgroundDrawnEvent(this));
   }
   ...
   private void openLink(URI p_openLink_1_) {
      Util.getPlatform().openUri(p_openLink_1_);
   }
   ...
   public static void wrapScreenError(Runnable p_wrapScreenError_0_, String p_wrapScreenError_1_, String p_wrapScreenError_2_) {
      try {
         ...
         CrashReport crashreport = CrashReport.forThrowable(throwable, p_wrapScreenError_1_);
