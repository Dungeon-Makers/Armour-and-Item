...
public class WinGameScreen extends Screen {
         ...
         try {
            ...
            if (this.poem) {
               ...
               while((s1 = bufferedreader.readLine()) != null) {
                  String s2;
                  String s3;
                  for(s1 = s1.replaceAll("PLAYERNAME", this.minecraft.getUser().getName()); s1.contains(s); s1 = s2 + TextFormatting.WHITE + TextFormatting.OBFUSCATED + "XXXXXXXX".substring(0, random.nextInt(4) + 3) + s3) {
                     int j = s1.indexOf(s);
                     s2 = s1.substring(0, j);
                     s3 = s1.substring(j + s.length());
                  }

                  this.lines.addAll(this.minecraft.font.func_78271_c(s1, 274));
                  this.lines.add("");
               }
            ...
            while((s4 = bufferedreader1.readLine()) != null) {
               s4 = s4.replaceAll("PLAYERNAME", this.minecraft.getUser().getName());
               s4 = s4.replaceAll("\t", "    ");
               this.lines.addAll(this.minecraft.font.func_78271_c(s4, 274));
               this.lines.add("");
            }
   ...
   public void render(int p_render_1_, int p_render_2_, float p_render_3_) {
      this.renderBg(p_render_1_, p_render_2_, p_render_3_);
      int i = 274;
      ...
      for(int i1 = 0; i1 < this.lines.size(); ++i1) {
         ...
         if ((float)l + f + 12.0F + 8.0F > 0.0F && (float)l + f < (float)this.height) {
            ...
            if (s.startsWith("[C]")) {
               this.font.func_175063_a(s.substring(3), (float)(j + (274 - this.font.width(s.substring(3))) / 2), (float)l, 16777215);
            } else {
               this.font.random.setSeed((long)((float)((long)i1 * 4238972211L) + this.time / 4.0F));
               this.font.func_175063_a(s, (float)j, (float)l, 16777215);
            }
         }
