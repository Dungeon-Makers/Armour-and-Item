...
public class ShaderInstance implements IShaderManager, AutoCloseable {
   ...
   public ShaderInstance(IResourceManager p_i50988_1_, String p_i50988_2_) throws IOException {
      ...
      try {
         ...
         String s1 = JSONUtils.getAsString(jsonobject, "fragment");
         ...
         this.field_216557_m = JSONUtils.getAsBoolean(jsonobject, "cull", true);
         this.vertexProgram = getOrCreate(p_i50988_1_, ShaderLoader.ShaderType.VERTEX, s);
         this.fragmentProgram = getOrCreate(p_i50988_1_, ShaderLoader.ShaderType.FRAGMENT, s1);
         this.programId = ShaderLinkHelper.createProgram();
         ShaderLinkHelper.linkProgram(this);
         this.updateLocations();
         if (this.attributeNames != null) {
            for(String s2 : this.attributeNames) {
               int l = ShaderUniform.glGetAttribLocation(this.programId, s2);
   ...
   public static ShaderLoader getOrCreate(IResourceManager p_216542_0_, ShaderLoader.ShaderType p_216542_1_, String p_216542_2_) throws IOException {
      ...
      if (shaderloader == null) {
         ...
         try {
            shaderloader = ShaderLoader.compileShader(p_216542_1_, p_216542_2_, iresource.getInputStream());
         } finally {
            IOUtils.closeQuietly((Closeable)iresource);
         }
      }
      ...
      for(int i = 0; i < this.samplerLocations.size(); ++i) {
         if (this.samplerMap.get(this.samplerNames.get(i)) != null) {
            ...
            if (object instanceof Framebuffer) {
               j = ((Framebuffer)object).colorTextureId;
            } else if (object instanceof Texture) {
               j = ((Texture)object).getId();
            ...
            if (j != -1) {
               ...
               ShaderUniform.uploadInteger(ShaderUniform.glGetUniformLocation(this.programId, this.samplerNames.get(i)), i);
            }
         }
      }
   ...
   private void updateLocations() {
      ...
      for(int j = 0; i < this.samplerNames.size(); ++j) {
         ...
         int k = ShaderUniform.glGetUniformLocation(this.programId, s);
         if (k == -1) {
   ...
   public void setSampler(String p_216537_1_, Object p_216537_2_) {
