...
public class ClientPlayerEntity extends AbstractClientPlayerEntity {
   ...
   public ClientPlayerEntity(Minecraft p_i50990_1_, ClientWorld p_i50990_2_, ClientPlayNetHandler p_i50990_3_, StatisticsManager p_i50990_4_, ClientRecipeBook p_i50990_5_) {
      super(p_i50990_2_, p_i50990_3_.getLocalGameProfile());
      this.connection = p_i50990_3_;
      this.stats = p_i50990_4_;
      this.recipeBook = p_i50990_5_;
      this.minecraft = p_i50990_1_;
      this.field_71093_bK = DimensionType.field_223227_a_;
      this.ambientSoundHandlers.add(new UnderwaterAmbientSoundHandler(this, p_i50990_1_.getSoundManager()));
   ...
   private void sendPosition() {
      ...
      if (this.isControlledCamera()) {
         ...
         if (this.isPassenger()) {
            ...
            this.connection.send(new CPlayerPacket.PositionRotationPacket(vec3d.x, -999.0D, vec3d.z, this.yRot, this.xRot, this.onGround));
            flag1 = false;
         } else if (flag1 && flag2) {
   ...
   public void removeRecipeHighlight(IRecipe<?> p_193103_1_) {
      if (this.recipeBook.willHighlight(p_193103_1_)) {
         ...
         this.connection.send(new CRecipeInfoPacket(p_193103_1_));
      }
   ...
   protected void moveTowardsClosestSpace(double p_213282_1_, double p_213282_3_, double p_213282_5_) {
      ...
      if (this.suffocatesAt(blockpos)) {
         ...
         double d2 = 9999.0D;
         if (!this.suffocatesAt(blockpos.west()) && d0 < d2) {
            d2 = d0;
            direction = Direction.WEST;
         }

         if (!this.suffocatesAt(blockpos.east()) && 1.0D - d0 < d2) {
            d2 = 1.0D - d0;
            direction = Direction.EAST;
         }

         if (!this.suffocatesAt(blockpos.north()) && d1 < d2) {
            d2 = d1;
            direction = Direction.NORTH;
         }

         if (!this.suffocatesAt(blockpos.south()) && 1.0D - d1 < d2) {
            d2 = 1.0D - d1;
         ...
         if (direction != null) {
            ...
            switch(direction) {
            case WEST:
               this.setDeltaMovement(-0.1D, vec3d.y, vec3d.z);
               break;
            case EAST:
               this.setDeltaMovement(0.1D, vec3d.y, vec3d.z);
               break;
            case NORTH:
               this.setDeltaMovement(vec3d.x, vec3d.y, -0.1D);
               break;
            case SOUTH:
               this.setDeltaMovement(vec3d.x, vec3d.y, 0.1D);
            }
   ...
   private boolean suffocatesAt(BlockPos p_205027_1_) {
      ...
      for(int i = MathHelper.floor(axisalignedbb.minY); i < MathHelper.ceil(axisalignedbb.maxY); ++i) {
         ...
         if (!this.freeAt(blockpos$mutable)) {
            return true;
         }
      }
   ...
   public void aiStep() {
      ...
      if (!this.noPhysics && !net.minecraftforge.common.MinecraftForge.EVENT_BUS.post(event)) {
         this.moveTowardsClosestSpace(this.getX() - (double)this.getBbWidth() * 0.35D, event.getMinY(), this.getZ() + (double)this.getBbWidth() * 0.35D);
         this.moveTowardsClosestSpace(this.getX() - (double)this.getBbWidth() * 0.35D, event.getMinY(), this.getZ() - (double)this.getBbWidth() * 0.35D);
         this.moveTowardsClosestSpace(this.getX() + (double)this.getBbWidth() * 0.35D, event.getMinY(), this.getZ() - (double)this.getBbWidth() * 0.35D);
         this.moveTowardsClosestSpace(this.getX() + (double)this.getBbWidth() * 0.35D, event.getMinY(), this.getZ() + (double)this.getBbWidth() * 0.35D);
      }
   ...
   private void handleNetherPortalClient() {
      ...
      if (this.isInsidePortal) {
         ...
         if (this.portalTime == 0.0F) {
            this.minecraft.getSoundManager().play(SimpleSound.forUI(SoundEvents.PORTAL_TRIGGER, this.random.nextFloat() * 0.4F + 0.8F));
         }
   ...
   protected void updateAutoJump(float p_189810_1_, float p_189810_2_) {
      if (this.canAutoJump()) {
         ...
         float f1 = (float)vec3d2.lengthSqr();
         if (f1 <= 0.001F) {
            Vec2f vec2f = this.input.getMoveVector();
            float f2 = f * vec2f.x;
            float f3 = f * vec2f.y;
         ...
         if (!(f13 < -0.15F)) {
            ...
            if (blockstate.getCollisionShape(this.level, blockpos, iselectioncontext).isEmpty()) {
               ...
               if (blockstate1.getCollisionShape(this.level, blockpos, iselectioncontext).isEmpty()) {
                  ...
                  AxisAlignedBB axisalignedbb = (new AxisAlignedBB(vec3d, vec3d4.add(0.0D, (double)f10, 0.0D))).inflate((double)f9, 0.0D, (double)f9);
                  Vec3d lvt_19_1_ = vec3d.add(0.0D, (double)0.51F, 0.0D);
                  vec3d4 = vec3d4.add(0.0D, (double)0.51F, 0.0D);
                  Vec3d vec3d5 = vec3d12.cross(new Vec3d(0.0D, 1.0D, 0.0D));
                  ...
                  Iterator<AxisAlignedBB> iterator = this.level.func_226667_c_(this, axisalignedbb, Collections.emptySet()).flatMap((p_212329_0_) -> {
