   ...
   private void tickNonPaused() {
         ...
         if (itickablesound.isStopped()) {
            ...
            if (channelmanager$entry != null) {
               channelmanager$entry.execute((p_217924_3_) -> {
                  ...
                  p_217924_3_.setSelfPosition(vec3d);
               });
            }
         }
      }
      ...
      while(iterator.hasNext()) {
         ...
         if (f2 <= 0.0F) {
            ...
            int j = this.soundDeleteTime.get(isound);
            if (j <= this.tickCount) {
               int i = isound.getDelay();
               if (isound.isLooping() && i > 0) {
                  this.queuedSounds.put(isound, this.tickCount + i);
               }
         ...
         if (soundeventaccessor == null) {
            ...
            if (this.listener.getGain() <= 0.0F) {
               ...
               if (sound == SoundHandler.EMPTY_SOUND) {
                  ...
                  float f1 = this.calculateVolume(p_148611_1_);
                  float f2 = this.calculatePitch(p_148611_1_);
                  ISound.AttenuationType isound$attenuationtype = p_148611_1_.getAttenuation();
                  boolean flag = p_148611_1_.isRelative();
                  if (f1 == 0.0F && !p_148611_1_.canStartSilent()) {
                     LOGGER.debug(MARKER, "Skipped playing sound {}, volume was zero.", (Object)sound.getLocation());
                  } else {
                     boolean flag1 = p_148611_1_.isLooping() && p_148611_1_.getDelay() == 0;
                     Vec3d vec3d = new Vec3d((double)p_148611_1_.getX(), (double)p_148611_1_.getY(), (double)p_148611_1_.getZ());
                     ChannelManager.Entry channelmanager$entry = this.channelAccess.func_217895_a(sound.shouldStream() ? SoundSystem.Mode.STREAMING : SoundSystem.Mode.STATIC);
                     ...
                     channelmanager$entry.execute((p_217930_7_) -> {
                        p_217930_7_.setPitch(f2);
                        p_217930_7_.setVolume(f1);
                        if (isound$attenuationtype == ISound.AttenuationType.LINEAR) {
                           p_217930_7_.linearAttenuation(f);
                        } else {
                           p_217930_7_.disableAttenuation();
                        }

                        p_217930_7_.setLooping(flag1);
                        p_217930_7_.setSelfPosition(vec3d);
                     ...
                     if (!sound.shouldStream()) {
                        ...
                        this.soundBuffers.getStream(sound.getPath()).thenAccept((p_217928_1_) -> {
