...
public class PropertyValueCondition implements ICondition {
   ...
   public Predicate<BlockState> getPredicate(StateContainer<Block, BlockState> p_getPredicate_1_) {
      ...
      if (iproperty == null) {
         ...
         if (list.isEmpty()) {
            ...
            if (list.size() == 1) {
               predicate = this.getBlockStatePredicate(p_getPredicate_1_, iproperty, s);
            } else {
               List<Predicate<BlockState>> list1 = list.stream().map((p_212482_3_) -> {
                  return this.getBlockStatePredicate(p_getPredicate_1_, iproperty, p_212482_3_);
   ...
   private Predicate<BlockState> getBlockStatePredicate(StateContainer<Block, BlockState> p_212485_1_, IProperty<?> p_212485_2_, String p_212485_3_) {
