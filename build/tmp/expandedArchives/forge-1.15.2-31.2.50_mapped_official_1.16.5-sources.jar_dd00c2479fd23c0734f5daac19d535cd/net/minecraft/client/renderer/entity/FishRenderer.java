...
public class FishRenderer extends EntityRenderer<FishingBobberEntity> {
   ...
   public void render(FishingBobberEntity p_225623_1_, float p_225623_2_, float p_225623_3_, MatrixStack p_225623_4_, IRenderTypeBuffer p_225623_5_, int p_225623_6_) {
      PlayerEntity playerentity = p_225623_1_.func_190619_l();
      if (playerentity != null) {
         ...
         double d6;
         float f3;
         if ((this.entityRenderDispatcher.options == null || this.entityRenderDispatcher.options.field_74320_O <= 0) && playerentity == Minecraft.getInstance().player) {
            double d7 = this.entityRenderDispatcher.options.fov;
            d7 = d7 / 100.0D;
            Vec3d vec3d = new Vec3d((double)i * -0.36D * d7, -0.045D * d7, 0.4D);
            vec3d = vec3d.xRot(-MathHelper.lerp(p_225623_3_, playerentity.xRotO, playerentity.xRot) * ((float)Math.PI / 180F));
            vec3d = vec3d.yRot(-MathHelper.lerp(p_225623_3_, playerentity.yRotO, playerentity.yRot) * ((float)Math.PI / 180F));
            vec3d = vec3d.yRot(f1 * 0.5F);
            vec3d = vec3d.xRot(-f1 * 0.7F);
            d4 = MathHelper.lerp((double)p_225623_3_, playerentity.xo, playerentity.getX()) + vec3d.x;
            d5 = MathHelper.lerp((double)p_225623_3_, playerentity.yo, playerentity.getY()) + vec3d.y;
            d6 = MathHelper.lerp((double)p_225623_3_, playerentity.zo, playerentity.getZ()) + vec3d.z;
