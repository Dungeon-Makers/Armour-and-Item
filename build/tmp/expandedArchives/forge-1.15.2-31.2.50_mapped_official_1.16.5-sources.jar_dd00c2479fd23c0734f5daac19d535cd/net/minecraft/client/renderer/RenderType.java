...
public abstract class RenderType extends RenderState {
   ...
   private static final RenderType LIGHTNING = create("lightning", DefaultVertexFormats.POSITION_COLOR, 7, 256, false, true, RenderType.State.builder().setWriteMaskState(COLOR_WRITE).setTransparencyState(LIGHTNING_TRANSPARENCY).setShadeModelState(SMOOTH_SHADE).createCompositeState(false));
   public static final RenderType.Type LINES = create("lines", DefaultVertexFormats.POSITION_COLOR, 1, 256, RenderType.State.builder().setLineState(new RenderState.LineState(OptionalDouble.empty())).setLayeringState(field_228500_J_).setTransparencyState(TRANSLUCENT_TRANSPARENCY).setWriteMaskState(COLOR_WRITE).createCompositeState(false));
   ...
   private static RenderType.State translucentState() {
      return RenderType.State.builder().setShadeModelState(SMOOTH_SHADE).setLightmapState(LIGHTMAP).setTextureState(BLOCK_SHEET_MIPPED).setTransparencyState(TRANSLUCENT_TRANSPARENCY).createCompositeState(true);
   }
   ...
   public static RenderType func_228635_a_(ResourceLocation p_228635_0_, float p_228635_1_) {
      RenderType.State rendertype$state = RenderType.State.builder().setTextureState(new RenderState.TextureState(p_228635_0_, false, false)).setAlphaState(new RenderState.AlphaState(p_228635_1_)).setCullState(NO_CULL).createCompositeState(true);
   ...
   static final class Type extends RenderType {
      ...
      private Type(String p_i225993_1_, VertexFormat p_i225993_2_, int p_i225993_3_, int p_i225993_4_, boolean p_i225993_5_, boolean p_i225993_6_, RenderType.State p_i225993_7_) {
         ...
         this.outline = p_i225993_7_.outlineProperty == RenderType.OutlineState.AFFECTS_OUTLINE ? p_i225993_7_.textureState.texture().map(RenderType::outline) : Optional.empty();
