...
public class DebugRenderer {
   ...
   public static Optional<Entity> getTargetedEntity(@Nullable Entity p_217728_0_, int p_217728_1_) {
      if (p_217728_0_ == null) {
         ...
         Vec3d vec3d1 = p_217728_0_.getViewVector(1.0F).scale((double)p_217728_1_);
         Vec3d vec3d2 = vec3d.add(vec3d1);
         AxisAlignedBB axisalignedbb = p_217728_0_.getBoundingBox().expandTowards(vec3d1).inflate(1.0D);
         int i = p_217728_1_ * p_217728_1_;
         Predicate<Entity> predicate = (p_217727_0_) -> {
            return !p_217727_0_.isSpectator() && p_217727_0_.isPickable();
         };
         EntityRayTraceResult entityraytraceresult = ProjectileHelper.getEntityHitResult(p_217728_0_, vec3d, vec3d2, axisalignedbb, predicate, (double)i);
         if (entityraytraceresult == null) {
            return Optional.empty();
         } else {
            return vec3d.distanceToSqr(entityraytraceresult.getLocation()) > (double)i ? Optional.empty() : Optional.of(entityraytraceresult.getEntity());
         }
      }
   }
   ...
   public static void renderFilledBox(BlockPos p_217735_0_, BlockPos p_217735_1_, float p_217735_2_, float p_217735_3_, float p_217735_4_, float p_217735_5_) {
      ...
      if (activerenderinfo.isInitialized()) {
         Vec3d vec3d = activerenderinfo.getPosition().reverse();
         AxisAlignedBB axisalignedbb = (new AxisAlignedBB(p_217735_0_, p_217735_1_)).move(vec3d);
   ...
   public static void renderFilledBox(BlockPos p_217736_0_, float p_217736_1_, float p_217736_2_, float p_217736_3_, float p_217736_4_, float p_217736_5_) {
      ...
      if (activerenderinfo.isInitialized()) {
         Vec3d vec3d = activerenderinfo.getPosition().reverse();
         AxisAlignedBB axisalignedbb = (new AxisAlignedBB(p_217736_0_)).move(vec3d).inflate((double)p_217736_1_);
