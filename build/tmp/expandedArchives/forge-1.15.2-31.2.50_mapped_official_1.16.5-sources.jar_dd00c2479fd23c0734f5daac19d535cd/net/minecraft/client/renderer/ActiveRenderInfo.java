...
public class ActiveRenderInfo {
   ...
   private double getMaxZoom(double p_216779_1_) {
      for(int i = 0; i < 8; ++i) {
         ...
         f1 = f1 * 0.1F;
         f2 = f2 * 0.1F;
         Vec3d vec3d = this.position.add((double)f, (double)f1, (double)f2);
         Vec3d vec3d1 = new Vec3d(this.position.x - (double)this.forwards.x() * p_216779_1_ + (double)f + (double)f2, this.position.y - (double)this.forwards.y() * p_216779_1_ + (double)f1, this.position.z - (double)this.forwards.z() * p_216779_1_ + (double)f2);
         RayTraceResult raytraceresult = this.level.clip(new RayTraceContext(vec3d, vec3d1, RayTraceContext.BlockMode.COLLIDER, RayTraceContext.FluidMode.NONE, this.entity));
   ...
   protected void move(double p_216782_1_, double p_216782_3_, double p_216782_5_) {
      ...
      this.setPosition(new Vec3d(this.position.x + d0, this.position.y + d1, this.position.z + d2));
   }
   ...
   protected void setPosition(double p_216775_1_, double p_216775_3_, double p_216775_5_) {
      this.setPosition(new Vec3d(p_216775_1_, p_216775_3_, p_216775_5_));
   }

   protected void setPosition(Vec3d p_216774_1_) {
   ...
   public IFluidState getFluidInCamera() {
      if (!this.initialized) {
         return Fluids.EMPTY.defaultFluidState();
      } else {
         IFluidState ifluidstate = this.level.getFluidState(this.blockPosition);
         return !ifluidstate.isEmpty() && this.position.y >= (double)((float)this.blockPosition.getY() + ifluidstate.getHeight(this.level, this.blockPosition)) ? Fluids.EMPTY.defaultFluidState() : ifluidstate;
      }
   }
