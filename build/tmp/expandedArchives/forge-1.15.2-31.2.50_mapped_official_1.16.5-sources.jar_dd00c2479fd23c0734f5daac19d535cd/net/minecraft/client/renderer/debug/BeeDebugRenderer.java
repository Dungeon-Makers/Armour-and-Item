...
public class BeeDebugRenderer implements DebugRenderer.IDebugRenderer {
   ...
   private void renderFlowerInfos() {
      Map<BlockPos, Set<UUID>> map = Maps.newHashMap();
      this.beeInfosPerEntity.values().stream().filter(BeeDebugRenderer.Bee::hasFlower).forEach((p_228979_1_) -> {
         Set<UUID> set = map.get(p_228979_1_.flowerPos);
         if (set == null) {
            set = Sets.newHashSet();
            map.put(p_228979_1_.flowerPos, set);
         }

         set.add(p_228979_1_.getUuid());
      });
   ...
   private Map<BlockPos, List<String>> getGhostHives() {
      ...
      for(BeeDebugRenderer.Bee beedebugrenderer$bee : this.beeInfosPerEntity.values()) {
         if (beedebugrenderer$bee.hivePos != null && !this.hives.containsKey(beedebugrenderer$bee.hivePos)) {
            List<String> list = map.get(beedebugrenderer$bee.hivePos);
            if (list == null) {
               list = Lists.newArrayList();
               map.put(beedebugrenderer$bee.hivePos, list);
            }

            list.add(beedebugrenderer$bee.getName());
         }
      }
