...
public class ClientLoginNetHandler implements IClientLoginNetHandler {
   ...
   public void handleHello(SEncryptionRequestPacket p_147389_1_) {
      ...
      String s = (new BigInteger(CryptManager.digestData(p_147389_1_.getServerId(), publickey, secretkey))).toString(16);
      CEncryptionResponsePacket cencryptionresponsepacket = new CEncryptionResponsePacket(secretkey, publickey, p_147389_1_.getNonce());
      ...
      HTTPUtil.DOWNLOAD_EXECUTOR.submit(() -> {
         ...
         this.connection.send(cencryptionresponsepacket, (p_209523_2_) -> {
            this.connection.func_150727_a(secretkey);
         });
      });
   }
   ...
   public void onDisconnect(ITextComponent p_147231_1_) {
      if (this.parent != null && this.parent instanceof RealmsScreenProxy) {
         this.minecraft.setScreen((new DisconnectedRealmsScreen(((RealmsScreenProxy)this.parent).getScreen(), "connect.failed", p_147231_1_)).getProxy());
      } else {
         this.minecraft.setScreen(new DisconnectedScreen(this.parent, "connect.failed", p_147231_1_));
      }
