...
public class PreYggdrasilConverter {
      ...
      if (OLD_USERBANLIST.exists() && OLD_USERBANLIST.isFile()) {
         if (banlist.getFile().exists()) {
            try {
               ...
               LOGGER.warn("Could not load existing file {}", banlist.getFile().getName(), filenotfoundexception);
            }
         }
      ...
      if (OLD_IPBANLIST.exists() && OLD_IPBANLIST.isFile()) {
         if (ipbanlist.getFile().exists()) {
            try {
               ...
               LOGGER.warn("Could not load existing file {}", ipbanlist.getFile().getName(), filenotfoundexception);
            }
         }
      ...
      if (OLD_OPLIST.exists() && OLD_OPLIST.isFile()) {
         if (oplist.getFile().exists()) {
            try {
               ...
               LOGGER.warn("Could not load existing file {}", oplist.getFile().getName(), filenotfoundexception);
            }
         }
      ...
      if (OLD_WHITELIST.exists() && OLD_WHITELIST.isFile()) {
         if (whitelist.getFile().exists()) {
            try {
               ...
               LOGGER.warn("Could not load existing file {}", whitelist.getFile().getName(), filenotfoundexception);
            }
         }
   ...
   public static String convertMobOwnerIfNecessary(final MinecraftServer p_187473_0_, String p_187473_1_) {
      if (!StringUtils.isNullOrEmpty(p_187473_1_) && p_187473_1_.length() <= 16) {
         ...
         if (gameprofile != null && gameprofile.getId() != null) {
            ...
            return PlayerEntity.createPlayerUUID(new GameProfile((UUID)null, p_187473_1_)).toString();
         }
      } else {
         return p_187473_1_;
      }
   ...
   public static boolean serverReadyAfterUserconversion(MinecraftServer p_219587_0_) {
      ...
      flag = flag && areOldPlayersConverted(p_219587_0_);
      return flag;
   }
