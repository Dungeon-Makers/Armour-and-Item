...
public class PointOfInterestType extends net.minecraftforge.registries.ForgeRegistryEntry<PointOfInterestType> {
   private static final Predicate<PointOfInterestType> ALL_JOBS = (p_221041_0_) -> {
      return Registry.VILLAGER_PROFESSION.stream().map(VillagerProfession::getJobPoiType).collect(Collectors.toSet()).contains(p_221041_0_);
   };
   public static final Predicate<PointOfInterestType> ALL = (p_221049_0_) -> {
      return true;
   };
   private static final Set<BlockState> BEDS = ImmutableList.of(Blocks.RED_BED, Blocks.BLACK_BED, Blocks.BLUE_BED, Blocks.BROWN_BED, Blocks.CYAN_BED, Blocks.GRAY_BED, Blocks.GREEN_BED, Blocks.LIGHT_BLUE_BED, Blocks.LIGHT_GRAY_BED, Blocks.LIME_BED, Blocks.MAGENTA_BED, Blocks.ORANGE_BED, Blocks.PINK_BED, Blocks.PURPLE_BED, Blocks.WHITE_BED, Blocks.YELLOW_BED).stream().flatMap((p_221043_0_) -> {
      return p_221043_0_.getStateDefinition().getPossibleStates().stream();
   }).filter((p_221050_0_) -> {
      return p_221050_0_.getValue(BedBlock.PART) == BedPart.HEAD;
   ...
   private static PointOfInterestType registerBlockStates(PointOfInterestType p_221052_0_) {
      p_221052_0_.matchingStates.forEach((p_221040_1_) -> {
         PointOfInterestType pointofinteresttype = TYPE_BY_STATE.put(p_221040_1_, p_221052_0_);
         if (pointofinteresttype != null) {
            throw (IllegalStateException)Util.pauseInIde(new IllegalStateException(String.format("%s is defined in too many tags", p_221040_1_)));
         }
      });
      return p_221052_0_;
   }
