...
public abstract class VoxelShapePart {
   ...
   @OnlyIn(Dist.CLIENT)
   public int firstFull(Direction.Axis p_197826_1_, int p_197826_2_, int p_197826_3_) {
      int i = this.getSize(p_197826_1_);
      if (p_197826_2_ >= 0 && p_197826_3_ >= 0) {
         Direction.Axis direction$axis = AxisRotation.FORWARD.cycle(p_197826_1_);
         Direction.Axis direction$axis1 = AxisRotation.BACKWARD.cycle(p_197826_1_);
         if (p_197826_2_ < this.getSize(direction$axis) && p_197826_3_ < this.getSize(direction$axis1)) {
            AxisRotation axisrotation = AxisRotation.between(Direction.Axis.X, p_197826_1_);

            for(int j = 0; j < i; ++j) {
               if (this.isFull(axisrotation, j, p_197826_2_, p_197826_3_)) {
                  return j;
               }
            }
