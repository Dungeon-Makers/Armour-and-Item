...
public abstract class VoxelShape {
   ...
   public double min(Direction.Axis p_197764_1_, double p_197764_2_, double p_197764_4_) {
      Direction.Axis direction$axis = AxisRotation.FORWARD.cycle(p_197764_1_);
      Direction.Axis direction$axis1 = AxisRotation.BACKWARD.cycle(p_197764_1_);
      int i = this.findIndex(direction$axis, p_197764_2_);
      int j = this.findIndex(direction$axis1, p_197764_4_);
      int k = this.shape.firstFull(p_197764_1_, i, j);
      return k >= this.shape.getSize(p_197764_1_) ? Double.POSITIVE_INFINITY : this.get(p_197764_1_, k);
   }
   ...
   @Nullable
   public BlockRayTraceResult clip(Vec3d p_212433_1_, Vec3d p_212433_2_, BlockPos p_212433_3_) {
      if (this.isEmpty()) {
         return null;
      } else {
         ...
         if (vec3d.lengthSqr() < 1.0E-7D) {
            ...
            return this.isFullWide(vec3d1.x - (double)p_212433_3_.getX(), vec3d1.y - (double)p_212433_3_.getY(), vec3d1.z - (double)p_212433_3_.getZ()) ? new BlockRayTraceResult(vec3d1, Direction.getNearest(vec3d.x, vec3d.y, vec3d.z).getOpposite(), p_212433_3_, true) : AxisAlignedBB.clip(this.toAabbs(), p_212433_1_, p_212433_2_, p_212433_3_);
         }
      }
   }
