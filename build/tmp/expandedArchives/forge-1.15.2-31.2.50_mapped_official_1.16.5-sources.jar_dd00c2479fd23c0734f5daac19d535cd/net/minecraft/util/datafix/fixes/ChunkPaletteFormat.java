...
public class ChunkPaletteFormat extends DataFix {
   ...
   public static int idFor(IntIdentityHashBiMap<Dynamic<?>> p_209724_0_, Dynamic<?> p_209724_1_) {
      int i = p_209724_0_.func_186815_a(p_209724_1_);
      if (i == -1) {
      ...
      public Dynamic<?> write() {
         ...
         if (!this.hasData) {
            ...
            for(int j = 0; j < this.buffer.length; ++j) {
               bitarray.set(j, this.buffer[j]);
            }

            dynamic = dynamic.set("BlockStates", dynamic.createLongList(Arrays.stream(bitarray.getRaw())));
      ...
      public UpgradeChunk(Dynamic<?> p_i49574_1_) {
         ...
         p_i49574_1_.get("TileEntities").asStreamOpt().ifPresent((p_210061_1_) -> {
            p_210061_1_.forEach((p_210063_1_) -> {
               ...
               int k4 = i4 << 8 | j4 << 4 | l3;
               if (this.blockEntities.put(k4, p_210063_1_) != null) {
                  ChunkPaletteFormat.LOGGER.warn("In chunk: {}x{} found a duplicate block entity at position: [{}, {}, {}]", this.x, this.z, l3, i4, j4);
               }

            });
         });
         boolean flag = p_i49574_1_.get("convertedFromAlphaFormat").asBoolean(false);
