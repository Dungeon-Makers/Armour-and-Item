...
public class UpgradeData {
      ...
      if (p_196991_0_.getUpgradeData().sides.remove(p_196991_1_)) {
         ...
         for(BlockPos blockpos : BlockPos.betweenClosed(k, 0, i1, l, world.getMaxBuildHeight() - 1, j1)) {
            ...
            for(Direction direction : adirection) {
               blockpos$mutable.set(blockpos).move(direction);
   ...
   private void upgradeInside(Chunk p_196989_1_) {
      try (
         BlockPos.PooledMutable blockpos$pooledmutable = BlockPos.PooledMutable.func_185346_s();
         BlockPos.PooledMutable blockpos$pooledmutable1 = BlockPos.PooledMutable.func_185346_s();
      ) {
         ...
         for(int i = 0; i < 16; ++i) {
            ...
            if (chunksection != null && aint != null && aint.length > 0) {
               ...
               for(int j : aint) {
                  ...
                  int l = j >> 8 & 15;
                  int i1 = j >> 4 & 15;
                  blockpos$pooledmutable.set(chunkpos.getMinBlockX() + k, (i << 4) + l, chunkpos.getMinBlockZ() + i1);
                  BlockState blockstate = palettedcontainer.get(j);
                  BlockState blockstate1 = blockstate;

                  for(Direction direction : adirection) {
                     blockpos$pooledmutable1.set(blockpos$pooledmutable).move(direction);
                     if (blockpos$pooledmutable.getX() >> 4 == chunkpos.x && blockpos$pooledmutable.getZ() >> 4 == chunkpos.z) {
                        blockstate1 = updateState(blockstate1, direction, iworld, blockpos$pooledmutable, blockpos$pooledmutable1);
                     }
                  }

                  Block.updateOrDestroy(blockstate, blockstate1, iworld, blockpos$pooledmutable, 18);
               }
            }
         }
   ...
   static enum BlockFixers implements UpgradeData.IBlockFixer {
      ...
      CHEST(Blocks.CHEST, Blocks.TRAPPED_CHEST) {
         public BlockState updateShape(BlockState p_196982_1_, Direction p_196982_2_, BlockState p_196982_3_, IWorld p_196982_4_, BlockPos p_196982_5_, BlockPos p_196982_6_) {
            if (p_196982_3_.getBlock() == p_196982_1_.getBlock() && p_196982_2_.getAxis().isHorizontal() && p_196982_1_.getValue(ChestBlock.TYPE) == ChestType.SINGLE && p_196982_3_.getValue(ChestBlock.TYPE) == ChestType.SINGLE) {
                  ...
                  if (blockstate.getValue(BlockStateProperties.DISTANCE) >= j) {
                     ...
                     if (i != 7) {
                        for(Direction direction : DIRECTIONS) {
                           ...
                           if (blockstate1.func_196959_b(BlockStateProperties.DISTANCE) && blockstate.getValue(BlockStateProperties.DISTANCE) > i) {
