...
public class CampfireBlock extends ContainerBlock implements IWaterLoggable {
   ...
   public void entityInside(BlockState p_196262_1_, World p_196262_2_, BlockPos p_196262_3_, Entity p_196262_4_) {
      if (!p_196262_4_.func_70045_F() && p_196262_1_.getValue(LIT) && p_196262_4_ instanceof LivingEntity && !EnchantmentHelper.hasFrostWalker((LivingEntity)p_196262_4_)) {
   ...
   public void animateTick(BlockState p_180655_1_, World p_180655_2_, BlockPos p_180655_3_, Random p_180655_4_) {
      if (p_180655_1_.getValue(LIT)) {
         if (p_180655_4_.nextInt(10) == 0) {
            p_180655_2_.playLocalSound((double)((float)p_180655_3_.getX() + 0.5F), (double)((float)p_180655_3_.getY() + 0.5F), (double)((float)p_180655_3_.getZ() + 0.5F), SoundEvents.CAMPFIRE_CRACKLE, SoundCategory.BLOCKS, 0.5F + p_180655_4_.nextFloat(), p_180655_4_.nextFloat() * 0.7F + 0.6F, false);
         }
      ...
      if (!p_204509_3_.getValue(BlockStateProperties.WATERLOGGED) && p_204509_4_.getType() == Fluids.WATER) {
         ...
         if (flag) {
            if (p_204509_1_.isClientSide()) {
               for(int i = 0; i < 20; ++i) {
                  makeParticles(p_204509_1_.getLevel(), p_204509_2_, p_204509_3_.getValue(SIGNAL_FIRE), true);
               }
            } else {
               p_204509_1_.playSound((PlayerEntity)null, p_204509_2_, SoundEvents.GENERIC_EXTINGUISH_FIRE, SoundCategory.BLOCKS, 1.0F, 1.0F);
            }

            TileEntity tileentity = p_204509_1_.getBlockEntity(p_204509_2_);
            if (tileentity instanceof CampfireTileEntity) {
   ...
   public void onProjectileHit(World p_220066_1_, BlockState p_220066_2_, BlockRayTraceResult p_220066_3_, Entity p_220066_4_) {
      if (!p_220066_1_.isClientSide) {
         boolean flag = p_220066_4_ instanceof AbstractFireballEntity || p_220066_4_ instanceof AbstractArrowEntity && p_220066_4_.isOnFire();
         if (flag) {
            ...
            if (flag1 && !p_220066_2_.getValue(LIT) && !p_220066_2_.getValue(WATERLOGGED)) {
   ...
   public static boolean func_226914_b_(World p_226914_0_, BlockPos p_226914_1_, int p_226914_2_) {
      for(int i = 1; i <= p_226914_2_; ++i) {
         BlockPos blockpos = p_226914_1_.below(i);
         BlockState blockstate = p_226914_0_.getBlockState(blockpos);
         if (isLitCampfire(blockstate)) {
            return true;
         }

         boolean flag = VoxelShapes.joinIsNotEmpty(VIRTUAL_FENCE_POST, blockstate.getCollisionShape(p_226914_0_, p_226914_1_, ISelectionContext.empty()), IBooleanFunction.AND);
         if (flag) {
            BlockState blockstate1 = p_226914_0_.getBlockState(blockpos.below());
   ...
   private static boolean isLitCampfire(BlockState p_226915_0_) {
      return p_226915_0_.getBlock() == Blocks.CAMPFIRE && p_226915_0_.getValue(LIT);
   }
